...
scriptName = "Automate LOG Se Deployer"
...
[5] = function()
   ...
   Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[5] ) } )
	Zoom( { width = 35000 } )
    actions:IssueOrder( "Automate LOG Se Deployer" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate LOG Se Deployer" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate LOG Se Deployer" then
					runStep()
				end
			end
		},
