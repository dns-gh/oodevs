...
scriptName = "Automate LOG Reconnaitre Zone De Deploiement"
...
[5] = function()
   ...
   Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[5] ) } )
	Zoom( { width = 35000 } )
    actions:IssueOrder( "Automate LOG Reconnaitre Zone De Deploiement" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate LOG Reconnaitre Zone De Deploiement" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate LOG Reconnaitre Zone De Deploiement" then
					runStep()
				end
			end
		},
