...
scriptName = "Automate LOG Surveiller"
...
[5] = function()
   ...
   Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[5] ) } )
	Zoom( { width = 35000 } )
    actions:IssueOrder( "Automate LOG Surveiller" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate LOG Surveiller" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate LOG Surveiller" then
					runStep()
				end
			end
		},
