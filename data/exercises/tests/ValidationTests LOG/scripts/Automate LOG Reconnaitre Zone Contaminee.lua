...
scriptName = "Automate LOG Reconnaitre Zone Contaminee"
...
[5] = function()
   ...
   Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[5] ) } )
	Zoom( { width = 35000 } )
    actions:IssueOrder( "Automate LOG Reconnaitre Zone Contaminee" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate LOG Reconnaitre Zone Contaminee" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate LOG Reconnaitre Zone Contaminee" then
					runStep()
				end
			end
		},
