...
scriptName = "Automate NBC Armer Un Site De Decontamination"
...
[5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ASS[1] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate NBC Armer Un Site De Decontamination C1 - 1" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate NBC Armer Un Site De Decontamination" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate NBC Armer Un Site De Decontamination" then
					runStep()
				end
			end
		},
