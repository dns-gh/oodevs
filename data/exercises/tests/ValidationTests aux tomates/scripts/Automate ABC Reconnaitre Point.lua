...
scriptName = "Automate ABC Reconnaitre Point"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Reconnaitre Point C1" )
end,

[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[4] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Reconnaitre Point C2 - 1" )
end,
[2000+20] = function()
	actions:IssueOrder( "Automate ABC Reconnaitre Point C2 - 2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate ABC Reconnaitre Point" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate ABC Reconnaitre Point" then
					runStep()
				end
			end
		},
