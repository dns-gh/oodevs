...
scriptName = "Automate ABC Recueillir Unite"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Recueillir Unite C1 - 1" )
end,
[150] = function()
	actions:IssueOrder( "Automate ABC Recueillir Unite C1 - 2" )
end,

[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Recueillir Unite C2 - 1" )
end,
[2000+150] = function()
	actions:IssueOrder( "Automate ABC Recueillir Unite C2 - 2" )
end,

[4000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[3] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Recueillir Unite C3 - 1" )
end,
[4000+40] = function()
	actions:IssueOrder( "Automate ABC Recueillir Unite C3 - 2" )
end,
[4000+210] = function()
	actions:IssueOrder( "Automate ABC Recueillir Unite C3 - 3" )
end,
}
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate ABC Recueillir Unite" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate ABC Recueillir Unite" then
					runStep()
				end
			end
		},
