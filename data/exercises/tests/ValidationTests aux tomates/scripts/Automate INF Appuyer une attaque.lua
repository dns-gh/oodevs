...
scriptName = "Automate INF Appuyer Une Attaque"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Appuyer Une Attaque C1 - 1" )
end,

[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Appuyer Une Attaque C2 - 1" )
end,
[2000+30] = function()
	actions:IssueOrder( "Automate INF Appuyer Une Attaque C2 - 2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate INF Appuyer Une Attaque" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate INF Appuyer Une Attaque" then
					runStep()
				end
			end
		},
