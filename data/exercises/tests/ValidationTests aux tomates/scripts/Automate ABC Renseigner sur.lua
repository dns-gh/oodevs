...
scriptName = "Automate ABC Renseigner Sur"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Renseigner Sur C1" )
end,

[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate ABC Renseigner Sur C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate ABC Renseigner Sur" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate ABC Renseigner Sur" then
					runStep()
				end
			end
		},
