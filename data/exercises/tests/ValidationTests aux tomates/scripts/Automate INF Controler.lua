...
scriptName = "Automate ABC Couvrir"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Controler C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Controler C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate INF Controler" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate INF Controler" then
					runStep()
				end
			end
		},
