...
scriptName = "Automate INF Semparer"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Semparer C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Semparer C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate INF Semparer" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate INF Semparer" then
					runStep()
				end
			end
		},
