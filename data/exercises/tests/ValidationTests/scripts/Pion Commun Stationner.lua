...
scriptName = "Pion Commun Stationner"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Stationner Commun Stationner C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Stationner Commun Faire Mouvement C2" )
end,
[5+100+2000] = function()
	actions:IssueOrder( "Mission Stationner Commun Stationner C2" )
end,
}
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion Commun Stationner" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion Commun Stationner" then
					runStep()
				end
			end
		},
