...
scriptName = "Pion ABC Renseigner sur"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion ABC Renseigner Sur 1 - C1" )
end,

[130] = function()
	actions:IssueOrder( "Pion ABC Renseigner Sur 2 - C1" )
end,
...
[5+2000] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion ABC Renseigner Sur - C2" )
end,

[5+4000] = function()
	Select( { target = "id://agent/"..formation[3].automat[3].agent[2].id } )
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[3] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion ABC Renseigner Sur - C3" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC Renseigner sur" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC Renseigner sur" then
					runStep()
				end
			end
		},
