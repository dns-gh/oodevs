...
scriptName = "Pion ABC S'emparer d'une zone"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Semparer ABC Semparer Zone C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Semparer ABC Semparer Zone C2" )
end,
[5+4000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[3] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Semparer ABC Semparer Zone C3" )
end,
}
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC S'emparer d'une zone" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC S'emparer d'une zone" then
					runStep()
				end
			end
		},
