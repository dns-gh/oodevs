...
scriptName = "Pion Commun Decrocher"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion Decrocher C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion Decrocher C2" )
end,
}
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion Commun Decrocher" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion Commun Decrocher" then
					runStep()
				end
			end
		},
