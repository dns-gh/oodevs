...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Harceler INF Harceler C1" )
end,
[70] = function()
	actions:IssueOrder( "Mission Harceler Faire Mouvement C1" )
end,

[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Harceler INF Harceler C2" )
end,
[5+70+2000] = function()
	Zoom( { width = 25000 } )
	actions:IssueOrder( "Mission Harceler Faire Mouvement C2" )
end,

[5+6000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[4] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Harceler INF Harceler C4" )
end,
[5+70+6000] = function()
	Zoom( { width = 25000 } )
	actions:IssueOrder( "Mission Harceler Faire Mouvement C4" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion INF Harceler" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion INF Harceler" then
					runStep()
				end
			end
		},
