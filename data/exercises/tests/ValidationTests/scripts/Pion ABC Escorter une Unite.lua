...
scriptName = "Pion ABC Escorter une unité"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission EscorterUnite Faire Mouvement C1" )
end,
[50] = function()
	actions:IssueOrder( "Mission EscorterUnite ABC Escorter Unite C1" )
end,
[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission EscorterUnite ABC Escorter Unite C2" )
end,
[4000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[3] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission EscorterUnite Faire Mouvement C3" )
end,
[4000+50+5] = function()
	actions:IssueOrder( "Mission EscorterUnite ABC Escorter Unite C3" )
end,
[6000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[4] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission EscorterUnite ABC Attaquer C4" )
end,
[6000+20+5] = function()
	actions:IssueOrder( "Mission EscorterUnite ABC Escorter Unite C4" )
end,
}
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC Escorter une unité" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC Escorter une unité" then
					runStep()
				end
			end
		},
