...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Appuyer ABC Attaquer C1" )
end,
[20] = function()
	actions:IssueOrder( "Mission Appuyer ABC Appuyer C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Appuyer ABC Attaquer C2" )
end,
[20+2000] = function()
	Zoom( { width = 25000 } )
	actions:IssueOrder( "Mission Appuyer ABC Appuyer C2" )
end,

[5+4000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[4] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Appuyer Faire Mouvement C3" )
	actions:IssueOrder( "Mission Appuyer ABC Attaquer C3" )
end,
[50+4000] = function()
	Zoom( { width = 25000 } )
	actions:IssueOrder( "Mission Appuyer ABC Appuyer C3" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC Appuyer" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC Appuyer" then
					runStep()
				end
			end
		},
