...
scriptName = "Pion ABC Controler zone"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ControlerZone ABC Controler Zone C1" )
end,
[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ControlerZone ABC Controler Zone C2" )
end,
[4000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[3] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ControlerZone ABC Controler Zone C3" )
end,
[6000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[4] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ControlerZone ABC Controler Zone C4" )
end,
}
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC Controler zone" then return end
				tick=0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC Controler zone" then
					runStep()
				end
			end
		},
