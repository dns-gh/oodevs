...
scriptName = "Pion GENIE Reconnaitre zone"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission GEN Reconnaitre Zone C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Reconnaitre zone Faire Mouvement C2" )
end,
[5+50+2000] = function()
	actions:IssueOrder( "Mission GEN Reconnaitre Zone C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion GENIE Reconnaitre zone" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion GENIE Reconnaitre zone" then
					runStep()
				end
			end
		},
