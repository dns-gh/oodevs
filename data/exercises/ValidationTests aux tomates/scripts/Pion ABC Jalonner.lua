...
[5] = function()
	Select( { target = "id://agent/"..formation[1].automat[3].agent[2].id } )
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[1] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion ABC Jalonner 1 - C1 bis" )
end,

[120] = function()
	actions:IssueOrder( "Pion ABC Jalonner 2 - C1 bis" )
end,

[130] = function()
	actions:IssueOrder( "Pion ABC Jalonner 3 - C1 bis" )
end,

[2000+5] = function()
	Select( { target = "id://agent/"..formation[2].automat[3].agent[2].id } )
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion ABC Jalonner 1 - C2 bis" )
end,

[2000+120] = function()
	actions:IssueOrder( "Pion ABC Jalonner 2 - C2 bis" )
end,

[2000+130] = function()
	actions:IssueOrder( "Pion ABC Jalonner 3 - C2 bis" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC Jalonner" then return end
				tick=0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC Jalonner" then
					runStep()
				end
			end
		},
