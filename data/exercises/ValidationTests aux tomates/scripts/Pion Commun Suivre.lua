...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission Commun Faire Mouvement C1" )
	actions:IssueOrder( "Mission Commun Suivre C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion CommunFaire Suivre Faire Mouvement ennemi C2" )
end,
[5+250+2000] = function()
	actions:IssueOrder( "Pion CommunFaire Suivre Faire Mouvement ami C2" )
	actions:IssueOrder( "Pion CommunFaire Suivre Suivre C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion Commun Suivre" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion Commun Suivre" then
					runStep()
				end
			end
		},
