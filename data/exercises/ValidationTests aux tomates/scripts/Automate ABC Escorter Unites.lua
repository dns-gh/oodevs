...
scriptName = "Automate ABC Escorter des unites"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Escorter Unites Faire Mouvement C1" )
end,
[5+15] = function()
	actions:IssueOrder( "Escorter Unites ABC Escorter Unites C1" )
end,
[5+2000] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Escorter Unites Faire Mouvement C2" )
end,
[5+15+2000] = function()
	actions:IssueOrder( "ABC Escorter Unites C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate ABC Escorter des unites" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate ABC Escorter des unites" then
					runStep()
				end
			end
		},
