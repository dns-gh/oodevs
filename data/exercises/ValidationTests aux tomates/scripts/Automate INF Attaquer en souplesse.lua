...
scriptName = "Automate INF Attaquer Souplesse"
...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Attaquer Souplesse C1 - 1" )
end,

[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Automate INF Attaquer Souplesse C2 - 1" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Automate INF Attaquer Souplesse" then return end
				tick = 0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Automate INF Attaquer Souplesse" then
					runStep()
				end
			end
		},
