...
[5] = function()
	...
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ArmerPIA ABC Armer PIA C1" )
end,
[100] = function()
	Zoom( { width = 15000 } )
	actions:IssueOrder( "Mission ArmerPIA Faire Mouvement C1" )
end,
[2000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ArmerPIA ABC Armer PIA C2" )
end,
[2000+70] = function()
	Zoom( { width = 15000 } )
	actions:IssueOrder( "Mission ArmerPIA Faire Mouvement C2" )
end,
[4000+5] = function()
	Center( { coordinates = coord:UtmPosition( config.positions.center_ABC[3] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Mission ArmerPIA ABC Armer PIA C3" )
end,
[4000+70] = function()
	Zoom( { width = 15000 } )
	actions:IssueOrder( "Mission ArmerPIA Faire Mouvement C3" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion ABC Armer PIA" then return end
				tick=0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion ABC Armer PIA" then
					runStep()
				end
			end
		},
