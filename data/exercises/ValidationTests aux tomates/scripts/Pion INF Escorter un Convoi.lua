...
scriptName = "Pion INF Escorter Unite"
...
[5] = function()
	Select( { target = "id://agent/"..formation[1].automat[1].agent[3].id } )
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[1] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion INF Escorter 1 - C1" )
end,

[30] = function()
	actions:IssueOrder( "Pion INF Escorter 2 - C1" )
end,

[2000+5] = function()
	Select( { target = "id://agent/"..formation[2].automat[1].agent[3].id } )
	Center( { coordinates = coord:UtmPosition( config.positions.center_INF[2] ) } )
	Zoom( { width = 35000 } )
	actions:IssueOrder( "Pion INF Escorter 1 - C2" )
end,

[2000+30] = function()
	actions:IssueOrder( "Pion INF Escorter 2 - C2" )
end,

[2000+60] = function()
	actions:IssueOrder( "Pion INF Escorter 3 - C2" )
end,
    ...
    {
		{
            ...
            function( client, profile )
                if profile ~= "Pion INF Escorter un convoi" then return end
				tick=0
		...
		{ "Missions_Init" },
			function()
				if profile ~= "Pion INF Escorter un convoi" then
					runStep()
				end
			end
		},
