...
integration.query.getFirstFireOrder = function( )
    ...
    for _,x in pairs( ordres_recus or emptyTable ) do
        if x:GetType() == "france.military.platoon.combat.support.art.tasks.AppliquerFeux" then
...
integration.GetSuperiorKnowledge = function( unit ) 
    local getAutomat = function() pcall( DEC_GetAutomate( unit.source ) ) end
    local result, automat = pcall( getAutomat )
    if not result then
        return false
    end
