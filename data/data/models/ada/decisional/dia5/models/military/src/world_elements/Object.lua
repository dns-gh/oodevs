return
{
    ...
    getProximity = function( self, target )
        if DEC_IsValidKnowledgeObject(self.source) then
            if masalife.brain.core.class.isOfType( target, world_elements.Direction ) then
    ...
    getPositionsToReconnoiter = function( self )
        ...
        for i = 1, #simPoints do
            res[ #res + 1 ] = CreateKnowledge( world_elements.Point, simPoints[i] ) -- create point as reconnoitring elements
        end
        return res
    end,
}
