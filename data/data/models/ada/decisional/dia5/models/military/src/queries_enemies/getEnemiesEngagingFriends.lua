    ...
    ["execute"] = function ( params )
        ...
        if knowledgeManager.bCanCallStaticQuery or params.dynamic then
            ...
            for _, entity in pairs( params.friends ) do
                res = {}
                entities = {}
                if masalife.brain.core.class.isOfType( entity, world_elements.Company ) then -- on peut appuyer des automates alliés
                ...
                for k = 1, #entities do
                    ...
                    for _, i in pairs( res ) do
                        if not exists( tableKn, i ) then
                            allRes[ #allRes + 1 ] = CreateKnowledge( world_elements.Platoon, i )
                            tableKn[ #tableKn ] = i
                        end
                    end
                end
