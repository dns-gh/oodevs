...
{ 
    [ "execute" ] = function ( params )
        ...
        for _, element in pairs ( params.elementsToReach ) do
            if masalife.brain.core.class.isOfType( element, world_elements.Object) and element:isAvoidable() then
                ...
                position = CreateKnowledge( world_elements.Point, objectPosition )
            else
        ...
        if not next( newResult ) then
            for _, elementToReach in pairs ( params.elementsToReach ) do
                ...
                for _, simPos in pairs( simPositions ) do
                    local kPos = CreateKnowledge( world_elements.Point, simPos )
