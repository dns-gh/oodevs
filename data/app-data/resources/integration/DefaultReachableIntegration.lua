...
integration.getAreaPosition = function( area )
    ...
    if integration.isPointInUrbanBlockTrafficable(CreateKnowledge( world_elements.Point, area.getAreaPositionResult), true ) == 0 then
...
integration.getAreaPositions = function( area )
    ...
    if #area.getTrafficableAreaPositionsResult == 0 then
        for i = 1, #area.getAreaPositionsResult do -- On ne veut que les positions qui sont dans la zone
            if integration.isPointInLocalisation(CreateKnowledge( world_elements.Point, area.getAreaPositionsResult[i] ), area) then
           ...
           and not myself.dismountedDone then
            ...
            if not DEC_Agent_EstDebarque() then
                ...
                if escort then -- Une unité m'escorte, la mission MoveToward devient StayClose
                    local escortUnit = CreateKnowledge( world_elements.PlatoonAlly, escort )
...
integration.isPointInUrbanBlockTrafficable = function( self, loaded )
    ...
    if not self.isPointInUrbanBlockTrafficableCache then
        local pos = self:getPosition()
        if masalife.brain.core.class.isOfType( meKnowledge, world_elements.Company  ) then 
...
integration.creerItineraireAPartirListePoint = function( listPoint )
    ...
    return CreateKnowledge( world_elements.Itinerary, DEC_CreerItineraireListe(listPointSource, eTypeItiMouvement) )
end
