    ...
    , radarZones_         ( RadarClass::GetNbrClasses() )
{
    ...
    entity[ "perceptions/localized-radars/radar" ].VisitChildren( boost::bind( &PerceptionRadar::EnableLocalizedRadar, this, _2, boost::cref( RadarClass::radar_ ) ) );
    entity[ "perceptions/localized-radars/tapping" ].VisitChildren( boost::bind( &PerceptionRadar::EnableLocalizedRadar, this, _2, boost::cref( RadarClass::tapping_ ) ) );
    entity[ "perceptions/localized-radars/tapping-radar" ].VisitChildren( boost::bind( &PerceptionRadar::EnableLocalizedRadar, this, _2, boost::cref( RadarClass::tappingRadar_ ) ) );
}
...
void PerceptionRadar::EnableLocalizedRadar( const wrapper::View& radar, const RadarClass& radarClass )
{
    ...
    radarId_[ radar[ "identifier" ] ] = zone;
}
...
void PerceptionRadar::Execute( const wrapper::View& model, const wrapper::View& perceiver, const SurfacesAgent_ABC& /*surfaces*/, const T_AgentPtrVector& /*perceivableAgents*/ )
{
    ...
    perceiver[ "perceptions/radars/acquisitions" ].VisitChildren( boost::bind( &::AddRadarType< T_RadarDataMap >, _1, boost::cref( perceiver ), boost::ref( effect ), boost::ref( radarData_ ) ) );
    ...
    for( RadarClass::CIT_RadarClassMap itRadarClass = radarClasses.begin(); itRadarClass != radarClasses.end(); ++itRadarClass )
    {
        ...
        if( !bRadarEnabledOnPerceiverPos && zones.empty() )
            continue;
        const T_RadarSet& radars = radarTypes_[ itRadarClass->second->GetID() ];
        for( T_RadarSet::const_iterator itRadar = radars.begin(); itRadar != radars.end(); ++itRadar )
        {
            const RadarType& radarType = **itRadar;
            T_RadarDataMap::iterator itRadarData = radarData_.find( &radarType );
            if( itRadarData == radarData_.end() )
            {
                itRadarData  = radarData_.insert( std::make_pair( &radarType, T_Data( new PerceptionRadarData( effect, radarType ) ) ) ).first;
                radarTypes_[ radarType.GetClass().GetID() ].insert( &radarType );
            }
