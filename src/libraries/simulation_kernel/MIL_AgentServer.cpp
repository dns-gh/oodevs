...
void MIL_AgentServer::WriteWeather( xml::xostream& xos ) const
{
    ...
    pMeteoDataManager_->WriteWeather( xos );
}
...
void MIL_AgentServer::SendControlInformation() const
{
    ...
    message().set_send_vision_cones( GetAgentServer().HasActiveVisionCones() );
...
// -----------------------------------------------------------------------------
void MIL_AgentServer::SetRealTime( const std::string& realTime, unsigned int context )
{
    unsigned int secs = 0;
