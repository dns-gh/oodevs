    ...
    public:
        ...
        void Notify() const
        {
            BOOST_FOREACH( const auto& link, initial_ )
                if( std::find_if( final_.begin(), final_.end(),
                        boost::bind( &HasSameSuperior, link, _1 ) ) == final_.end() )
                    owner_.NotifyLinkRemoved( *link );
            BOOST_FOREACH( const auto& link, final_ )
                if( std::find_if( initial_.begin(), initial_.end(),
                        boost::bind( &HasSameSuperior, link, _1 ) ) == initial_.end() )
