...
MIL_EntityManager::MIL_EntityManager( const MIL_Time_ABC& time, MIL_EffectManager& effects, MIL_ObjectFactory& objectFactory, const MIL_Config& config )
    ...
    , inhabitantFactory_            ( new InhabitantFactory( config ) )
    , populationFactory_            ( new PopulationFactory( *missionController_, gcPause_, gcMult_, config ) )
    , agentFactory_                 ( new AgentFactory( *idManager_, *missionController_, config ) )
