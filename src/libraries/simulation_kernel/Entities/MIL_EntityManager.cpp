...
MIL_Object_ABC* MIL_EntityManager::CreateObject( MIL_Army_ABC& army, const std::string& type, const TER_Localisation* pLocalisation,
                                                 ObstacleType_DemolitionTargetType obstacleType, unsigned int externalIdentifier, const std::string& name, double density )
{
    return pObjectManager_->CreateObject( army, type, pLocalisation, obstacleType, externalIdentifier, name, density );
}
...
void MIL_EntityManager::ProcessLogSupplyChangeQuotas( const UnitMagicAction& message, unsigned int nCtx )
{
    ...
    try
    {
        ...
        for( std::set< const PHY_DotationCategory* >::iterator typeIt = quotasTypes.begin(); typeIt != quotasTypes.end(); ++typeIt )
        {
            ...
            {
                MIL_Automate* pAutomateTC2 = TaskerToAutomat( *this, message.tasker() );
                if( pAutomateTC2 )
                    MIL_Report::PostEvent( *pAutomateTC2, MIL_Report::eRC_SupplierUnavailable );
                else if( MIL_Formation* pSuppliedFormation = TaskerToFormation( *this, message.tasker() ) )
                    MIL_Report::PostEvent( *pSuppliedFormation, MIL_Report::eRC_SupplierUnavailable );
                break;
            }
        }
    }
