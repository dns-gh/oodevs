...
void save_construct_data( Archive& archive, const PHY_RoleAction_Flying* role, const unsigned int /*version*/ )
{
    ...
    const moving::MoveComputerFactory_ABC const* moveComputerFactory = &role->moveComputerFactory_;
    archive << entity
            << moveComputerFactory;
}
...
void load_construct_data( Archive& archive, PHY_RoleAction_Flying* role, const unsigned int /*version*/ )
{
    ...
    moving::MoveComputerFactory_ABC* moveComputerFactory;
    archive >> entity
            >> moveComputerFactory;
    ::new( role )PHY_RoleAction_Flying( *entity, *moveComputerFactory );
}
...
// -----------------------------------------------------------------------------
PHY_RoleAction_Flying::PHY_RoleAction_Flying( MIL_Entity_ABC& entity, const moving::MoveComputerFactory_ABC& moveComputerFactory )
    : entity_                        ( entity )
    , effectFly_                    ( *this )
    , pActionFly_                   ( 0 )
    , moveComputerFactory_          ( moveComputerFactory )
{
    // NOTHING
}
...
{
    moving::MoveComputer_ABC& moveComputer = moveComputerFactory_.Create();//@TODO MGD see to share algorthms and execute them just one time by step
