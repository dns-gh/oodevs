...
double PHY_HumansComposante::GetOperationalState() const
{
    ...
    double state = 0;
    for( auto it = humans_.begin(); it != humans_.end(); ++it )
        if( !(*it)->IsSeriouslyPhysicallyWounded() && !(*it)->IsDead() && (*it)->GetLocation() == eHumanLocation_Battlefield )
            ++state;
...
bool PHY_HumansComposante::CanBeUsed() const
{
    for( auto it = humans_.begin(); it != humans_.end(); ++it )
    {
        if( (*it)->IsDead() || (*it)->IsSeriouslyPhysicallyWounded() || (*it)->GetLocation() != eHumanLocation_Battlefield )
            continue;
        else
            return true;
    }
