...
void PHY_HumanWound::ReadInjury( xml::xistream& xis, double& rFactorSum )
{
    ...
    if( it != humanWounds_.end() )
    {
        ...
        if( tools::ReadTimeAttribute( xis, "life-expectancy", rValue ) )
        {
            ...
            const_cast< PHY_HumanWound& >( wound ).nLifeExpectancy_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );
        }

        tools::ReadTimeAttribute( xis, "caring-time", rValue );
        if( rValue < 0 )
            xis.error( "injury: caring-time < 0" );
        const_cast< PHY_HumanWound& >( wound ).nHealingTime_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );

        tools::ReadTimeAttribute( xis, "resting-time", rValue );
        if( rValue < 0 )
            xis.error( "injury: resting-time < 0" );
        const_cast< PHY_HumanWound& >( wound ).nRestingTime_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );
    }
    else if( injuryType == "mental" )
    {
        ...
        if( rValue < 0 )
            xis.error( "injury: caring-time < 0" );
        nMentalDiseaseHealingTime_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );

        tools::ReadTimeAttribute( xis, "resting-time", rValue );
        if( rValue < 0 )
            xis.error( "injury: resting-time < 0" );
        nMentalDiseaseRestingTime_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );
    ...
    else if( injuryType == "contaminated" )
    {
        ...
        if( rValue < 0 )
            xis.error( "injury: caring-time < 0" );
        nContaminatedHealingTime_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );

        tools::ReadTimeAttribute( xis, "resting-time", rValue );
        if( rValue < 0 )
            xis.error( "injury: resting-time < 0" );
        nContaminatedRestingTime_ = (unsigned int)MIL_Tools::ConvertSecondsToSim( rValue );
    }
    else
