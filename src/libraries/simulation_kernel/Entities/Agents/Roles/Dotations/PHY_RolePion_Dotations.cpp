...
{
    assert( pDotations_ );
    pDotations_->ReadDotations( xis, pion_->GetType().GetUnitType() );
}
...
void PHY_RolePion_Dotations::NotifySupplyNeeded( const PHY_DotationCategory& dotationCategory, bool bNewNeed ) const
{
    ...
    if( bNewNeed )
    {
        MIL_Report::PostEvent( *pion_, MIL_Report::eRC_SeuilLogistiqueDotationDepasse, dotationCategory );
        if( pion_->GetType().IsRefugee() || pion_->GetRole< surrender::PHY_RoleInterface_Surrender >().IsPrisoner() )
            MIL_Report::PostEvent( *pion_, MIL_Report::eRC_PrisonersUnsupplied );
    }
...
{
    return pion_->GetAutomate().HasDotationSupplyNeededNotified( dotationCategory );
}
...
void PHY_RolePion_Dotations::SetForbiddenDotation( const PHY_DotationCategory& category )
{
    ...
    {
        MIL_Report::PostEvent( *pion_, MIL_Report::eRC_MunitionInterdite, category );
...
void PHY_RolePion_Dotations::RemoveForbiddenDotation( const PHY_DotationCategory& category )
{
    ...
    if( it != forbiddenDotations_.end() )
    {
       MIL_Report::PostEvent( *pion_, MIL_Report::eRC_MunitionAutorise, category );
...
{
    if( category.GetIndirectFireData() )
        return( GetDotationNumber( category ) >= category.GetIndirectFireData()->ConvertToNbrAmmo( static_cast< double >( iterations ) ) );
    return false;
}
