...
double DEC_Agent_PathfinderRule::GetCost( const MT_Vector2D& from, const MT_Vector2D& to, const TerrainData& nToTerrainType, const TerrainData& nLinkTerrainType, std::ostream* reason )
{
    ...
    if( rSpeed <= 0 )
    {
        LOG_REASON( "no way: speed on " << nLinkTerrainType.DumpToString()
                << " == " << rSpeed );
        return -1;
    }
    ...
    {
        LOG_REASON( "no way: slope is too steep" );
        return -1;
    }
    ...
    if( rSlopeDeceleration_ != 0 )
    {
        ...
        if( rSpeed == 0 )
        {
            LOG_REASON( "no way: speed on slope is null" );
            return -1;
        }
    }
    ...
    {
        LOG_REASON( "no way: urban block cost" );
        return -1;
    }
    ...
    if( rEnemiesCost < 0 )
    {
        LOG_REASON( "no way: enemies cost" );
        return -1;
    }
    ...
    {
        LOG_REASON( "no way: populations cost" );
        return -1;
    }
       ...
       std::ostream* reason ) const
{
    ...
    // Vérification si le link est dans le fuseau de l'agent
    {
        ...
        if( rFuseauCost < 0 )
        {
            LOG_REASON( "not way: unit limits" );
            return -1;
        }
        rCost += rFuseauCost;
    }
    // $$$$ AGE 2005-06-24: Going out of the automate fuseau is a no-no. Avoid precision issues
    {
        ...
        {
            LOG_REASON( "no way: automat limits" );
            return -1;
        }
