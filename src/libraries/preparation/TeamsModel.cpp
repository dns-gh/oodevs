...
// -----------------------------------------------------------------------------
Object_ABC* TeamsModel::CreateObject( const kernel::Team_ABC& team, kernel::ObjectType& type, const QString& name, bool prepare, const kernel::Location_ABC& location )
{
    return static_cast< Team& >( Get( team.GetId() ) ).CreateObject( type, name, prepare, location );
}
