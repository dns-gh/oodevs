...
void UrbanFileExporter::WriteObject( const kernel::UrbanObject_ABC& urbanObject, unsigned int counter )
{
    ...
    pFeature->SetField( "NAME", urbanObject.GetName().toUtf8().constData() );
    ...
    // Hierarchy
    {
        ...
        pFeature->SetField( "DIST_NAME", districtObject->GetName().toUtf8().constData() );
        ...
        pFeature->SetField( "TOWN_NAME", cityObject->GetName().toUtf8().constData() );
    }
