    ...
    , parameter_( parameter )
{
    ...
    {
        Parameter_ABC* param = factory.CreateParameter( OrderParameter( tools::translate( "Parameter", "%1 (item %2)" ).arg( parameter.GetName().c_str() ).arg( i + 1 ).toUtf8().constData(), "location", false ), *it, entity );
        if( param )
            AddParameter( *param );
        else
