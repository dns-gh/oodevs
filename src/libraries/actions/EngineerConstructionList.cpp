    ...
    : Parameter< std::string >( parameter )
{
    for( int i = 0; i < message.elem_size(); ++i )
        AddParameter( *new EngineerConstruction( OrderParameter( tools::translate( "Parameter", "%1 (item %2)" ).arg( parameter.GetName().c_str() ).arg( i + 1 ).ascii(), "genobject", false ), converter, resolver, entities, message.elem(i), controller ) );
}
