...
// -----------------------------------------------------------------------------
ProfileManager::ProfileManager( Master& master, const Config& config )
    : master_  ( master )
    , profiles_()
{
    xml::xifstream xis( config.GetMasterFile() );
...
void ProfileManager::ReadProfile( xml::xistream& xis )
{
    ...
    if( profiles_.Find( strName ) )
    {
        MT_LOG_ERROR_MSG( "Profile '" << strName << "' already exists - new profile ignored" );
    }
    else
        profiles_.Create( strName, master_, strName, xis );
}
...
{
    Profile* pProfile = profiles_.Find( strName );
    if( !pProfile )
    {
    ...
    if( !pProfile->CheckPassword( strPassword ) )
    {
