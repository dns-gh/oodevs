...
#ifndef plugins_hla_OtherPointObject_H_
...
class OtherPointObject : public HlaObject_ABC
...
public:
    ...
    //@{
    OtherPointObject( TacticalObject_ABC& object, unsigned int identifier, const std::string& name, rpr::ForceIdentifier force, const rpr::EntityType& type,
        const rpr::EntityIdentifier& entityId, const std::string& rtiId );
    OtherPointObject( const std::string& identifier, EntityIdentifierResolver_ABC& entityIdentifierResolver, FOM_Serializer_ABC& fomSerializer );
    virtual  ~OtherPointObject();
    //@}
