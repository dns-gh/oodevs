...
void Converter::ReceiveMessengerToClient( const sword::MessengerToClient& msg )
{
    ...
    FORWARD_TO( client_, MessengerToClient, marker_creation_request_ack, marker_creation_ack )
    FORWARD_TO( client_, MessengerToClient, marker_update_request_ack, marker_update_ack )
    FORWARD_TO( client_, MessengerToClient, marker_destruction_request_ack, marker_destruction_ack )
...
void Converter::ReceiveReplayToClient( const sword::ReplayToClient& msg )
{
    ...
    FORWARD( client_, ReplayToClient, control_send_current_state_begin )
    FORWARD( client_, ReplayToClient, control_send_current_state_end )
...
void Converter::ReceiveSimToClient( const sword::SimToClient& msg )
{
	...
	if ( msg.message().has_object_creation() )
	{
    ...
    FORWARD( client_, SimulationToClient, indirect_fire_perception )
    ...
    FORWARD( client_, SimulationToClient, log_funeral_handling_creation )
    FORWARD( client_, SimulationToClient, log_funeral_handling_update )
    FORWARD( client_, SimulationToClient, log_funeral_handling_destruction )
    ...
    FORWARD_TO( client_, SimulationToClient, knowledge_group_update_ack, knowledge_group_magic_action_ack )
...
void Converter::ReceiveClientToSim( const MsgsClientToSim::MsgClientToSim& msg )
{
    ...
    if( msg.message().has_unit_magic_action() && msg.message().unit_magic_action().type() == Common::knowledge_creation )
    {
        if ( msg.message().unit_magic_action().parameters().elem_size() == 2 )
        {
            out.mutable_message()->mutable_knowledge_magic_action()->set_type( sword::add_knowledge );
    ...
    FORWARD( server_, ClientToSimulation, log_history_request_for_play )
}
