...
EXTERN int asn1PE_MissionObjective (ASN1CTXT* ctxt_p, ASN1T_MissionObjective* pvalue)
{
   ...
   pe_bit (ctxt_p, (ASN1BOOL)(pvalue->horaire != 0));
   ...
   if (pvalue->horaire != 0) {
      stat = pe_UnconsInteger (ctxt_p, pvalue->horaire);
      if (stat != ASN_OK) return stat;
   }
...
EXTERN int asn1PD_MissionObjective (ASN1CTXT* ctxt_p, ASN1T_MissionObjective* pvalue)
{
   int stat = ASN_OK;
   ASN1BOOL optbit;
   ASN1BOOL horairePresent;

   /* optional bits */

   pd_bit (ctxt_p, &optbit);
   horairePresent = optbit;
   ...
   if (horairePresent) {
      stat = pd_UnconsInteger (ctxt_p, &pvalue->horaire);
      if (stat != ASN_OK) return stat;
   }
   else {
      pvalue->horaire = 0;
