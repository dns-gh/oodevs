...
#define TRI_PI 3.141592653589793238462643383279502884197169399375105820974944592308
...
typedef MT_Float **triangle;            /* MT_Floatly:  typedef triangle *triangle   */
...
typedef MT_Float **shelle;                  /* MT_Floatly:  typedef shelle *shelle   */
...
/* The point data structure.  Each point is actually an array of MT_Floats.      */
/*   The number of MT_Floats is unknown until runtime.  An integer boundary      */
/*   marker, and sometimes a pointer to a triangle, is appended after the    */
/*   MT_Floats.                                                                  */

typedef MT_Float *point;
...
struct badface {
  ...
  MT_Float key;                             /* cos^2 of smallest (apical) angle. */
...
struct event {
  MT_Float xkey, ykey;                              /* Coordinates of the event. */
...
MT_Float xmin, xmax, ymin, ymax;                              /* x and y bounds. */
MT_Float xminextreme;        /* Nonexistent x value used as a flag in sweepline. */
...
MT_Float splitter;       /* Used to split MT_Float factors for exact multiplication. */
MT_Float epsilon;                             /* Floating-point machine epsilon. */
MT_Float resulterrbound;
MT_Float ccwerrboundA, ccwerrboundB, ccwerrboundC;
MT_Float iccerrboundA, iccerrboundB, iccerrboundC;
...
/*   shell edge that isn't MT_Floatly connected to a shell edge at that          */
/*   location.                                                               */
...
#define elemattribute(triedge, attnum)                                        \
  ((MT_Float *) (triedge).tri)[elemattribindex + (attnum)]

#define setelemattribute(triedge, attnum, value)                              \
  ((MT_Float *) (triedge).tri)[elemattribindex + (attnum)] = value

/* Check or set a triangle's maximum area bound.                             */

#define areabound(triedge)  ((MT_Float *) (triedge).tri)[areaboundindex]

#define setareabound(triedge, value)                                          \
  ((MT_Float *) (triedge).tri)[areaboundindex] = value
...
/*   are masked out to produce the MT_Float pointer.                             */
...
#define STARTINDEX 0
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            minangle = (MT_Float) strtod(workstring, (char **) NULL);
	  } else {
            minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            maxarea = (MT_Float) strtod(workstring, (char **) NULL);
            if (maxarea <= 0.0)
  ...
  goodangle = cos(minangle * TRI_PI / 180.0);
  goodangle *= goodangle;
...
void poolinit( struct memorypool* pool, int bytecount, int itemcount, enum wordtype wtype, int alignment)
{
  ...
  wordsize = (pool->itemwordtype == POINTER) ? sizeof(VOID *) : sizeof(MT_Float);
  ...
  nSize = pool->itemsperblock * pool->itembytes + sizeof(VOID *) + pool->alignbytes;
  pool->firstblock = (VOID **) malloc( nSize );
  if (pool->firstblock == (VOID **) NULL) 
      throw std::exception("Error:  Out of memory.\n");
  memset( (void*)pool->firstblock, 0, nSize  );
  ...
  if (pool->deaditemstack != (VOID *) NULL) {
    ...
    if (pool->unallocateditems == 0) {
      ...
      if (*(pool->nowblock) == (VOID *) NULL) 
	  {
		...
		int nSize = pool->itemsperblock * pool->itembytes + sizeof(VOID *) + pool->alignbytes;
		newblock = (VOID **) malloc( nSize );
        if (newblock == (VOID **) NULL) 
            throw std::exception( "Error:  Out of memory." );
		memset( (void*)newblock, 0 , nSize );
    ...
    if (pool->itemwordtype == POINTER) {
      ...
      pool->nextitem = (VOID *) ((MT_Float *) pool->nextitem + pool->itemwords);
    }
...
VOID *traverse(struct memorypool *pool)
{
  ...
  if (pool->itemwordtype == POINTER) {
    ...
    pool->pathitem = (VOID *) ((MT_Float *) pool->pathitem + pool->itemwords);
  }
  pool->pathitemsleft--;
  return newitem;
}
...
/*  or shell edge that doesn't have a full complement of MT_Float shell edges    */
/*  to point to.                                                             */
/*                                                                           */
/*****************************************************************************/
...
void dummyinit( int trianglewords, int shellewords)
{
  ...
  /* Set up `dummytri', the `triangle' that occupies "outer space". */
	nSize = triwords * sizeof(triangle) + triangles.alignbytes;
  dummytribase = (triangle *) malloc( nSize );
  if (dummytribase == (triangle *) NULL) 
      throw std::exception( "Error:  Out of memory." );
  memset ( (void*)dummytribase, 0, nSize );
  ...
  /*   values don't MT_Floatly matter, as long as they can legally be          */
  /*   dereferenced.                                                       */
  ...
  if (useshelles) 
  {
    ...
    /*   triangle side or shell edge end that isn't attached to a MT_Float shell */
    /*   edge.                                                               */
		nSize = shwords * sizeof(shelle) + shelles.alignbytes;
	  dummyshbase = (shelle *) malloc( nSize );
    if (dummyshbase == (shelle *) NULL) 
        throw std::exception( "Error:  Out of memory." );
	memset( (void*)dummyshbase, 0 , nSize );
    ...
    /*   operations, but their values don't MT_Floatly matter, as long as they  */
...
void initializepointpool()
{
  ...
  pointmarkindex = ((mesh_dim + nextras) * sizeof(MT_Float) + sizeof(int) - 1)
                 / sizeof(int);
  ...
  poolinit(&points, pointsize, POINTPERBLOCK,
           (sizeof(MT_Float) >= sizeof(triangle)) ? FLOATINGPOINT : POINTER, 0);
}
...
void initializetrisegpools()
{
  ...
  /*   where the index is measured in MT_Floats.                           */
  elemattribindex = (trisize + sizeof(MT_Float) - 1) / sizeof(MT_Float);
  /* The index within each triangle at which the maximum area constraint  */
  /*   is found, where the index is measured in MT_Floats.  Note that if the  */
  /*   `regionattrib' flag is set, an additional attribute will be added. */
  areaboundindex = elemattribindex + eextras + regionattrib;
  /* If triangle attributes or an area bound are needed, increase the number */
  /*   of bytes occupied by a triangle.                                      */
  if (vararea) {
    trisize = (areaboundindex + 1) * sizeof(MT_Float);
  } else if (eextras + regionattrib > 0) {
    trisize = areaboundindex * sizeof(MT_Float);
  }
...
/*  floating-point arithmetic is too much like MT_Float arithmetic.              */
/*                                                                           */
...
int fast_expansion_sum_zeroelim( int elen, MT_Float *e,int  flen, MT_Float *f, MT_Float *h)  /* h cannot be e or f. */
{
  MT_Float Q;
  INEXACT MT_Float Qnew;
  INEXACT MT_Float hh;
  INEXACT MT_Float bvirt;
  MT_Float avirt, bround, around;
...
int scale_expansion_zeroelim(int elen, MT_Float *e, MT_Float b, MT_Float *h)   /* e and h cannot be the same. */
{
  INEXACT MT_Float Q, sum;
  MT_Float hh;
  INEXACT MT_Float product1;
  MT_Float product0;
  int eindex, hindex;
  MT_Float enow;
  INEXACT MT_Float bvirt;
  MT_Float avirt, bround, around;
  INEXACT MT_Float c;
  INEXACT MT_Float abig;
  MT_Float ahi, alo, bhi, blo;
  MT_Float err1, err2, err3;
...
MT_Float estimate(int elen, MT_Float *e)
{
  MT_Float Q;
  int eindex;
...
MT_Float counterclockwiseadapt(point pa, point pb, point pc, MT_Float detsum)
{
  INEXACT MT_Float acx, acy, bcx, bcy;
  MT_Float acxtail, acytail, bcxtail, bcytail;
  INEXACT MT_Float detleft, detright;
  MT_Float detlefttail, detrighttail;
  MT_Float det, errbound;
  MT_Float B[4], C1[8], C2[12], D[16];
  INEXACT MT_Float B3;
  int C1length, C2length, Dlength;
  MT_Float u[4];
  INEXACT MT_Float u3;
  INEXACT MT_Float s1, t1;
  MT_Float s0, t0;
  ...
  MT_Float ahi, alo, bhi, blo;
  MT_Float err1, err2, err3;
  INEXACT MT_Float _i, _j;
  MT_Float _0;

  acx = (MT_Float) (pa[0] - pc[0]);
  bcx = (MT_Float) (pb[0] - pc[0]);
  acy = (MT_Float) (pa[1] - pc[1]);
  bcy = (MT_Float) (pb[1] - pc[1]);
...
MT_Float counterclockwise(point pa, point pb, point pc)
{
  MT_Float detleft, detright, det;
  MT_Float detsum, errbound;
...
MT_Float incircleadapt(point pa, point pb, point pc, point pd, MT_Float permanent)
{
  INEXACT MT_Float adx, bdx, cdx, ady, bdy, cdy;
  MT_Float det, errbound;

  INEXACT MT_Float bdxcdy1, cdxbdy1, cdxady1, adxcdy1, adxbdy1, bdxady1;
  MT_Float bdxcdy0, cdxbdy0, cdxady0, adxcdy0, adxbdy0, bdxady0;
  MT_Float bc[4], ca[4], ab[4];
  INEXACT MT_Float bc3, ca3, ab3;
  MT_Float axbc[8], axxbc[16], aybc[8], ayybc[16], adet[32];
  int axbclen, axxbclen, aybclen, ayybclen, alen;
  MT_Float bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int bxcalen, bxxcalen, bycalen, byycalen, blen;
  MT_Float cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int cxablen, cxxablen, cyablen, cyyablen, clen;
  MT_Float abdet[64];
  int ablen;
  MT_Float fin1[1152], fin2[1152];
  MT_Float *finnow, *finother, *finswap;
  int finlength;

  MT_Float adxtail, bdxtail, cdxtail, adytail, bdytail, cdytail;
  INEXACT MT_Float adxadx1, adyady1, bdxbdx1, bdybdy1, cdxcdx1, cdycdy1;
  MT_Float adxadx0, adyady0, bdxbdx0, bdybdy0, cdxcdx0, cdycdy0;
  MT_Float aa[4], bb[4], cc[4];
  INEXACT MT_Float aa3, bb3, cc3;
  INEXACT MT_Float ti1, tj1;
  MT_Float ti0, tj0;
  MT_Float u[4], v[4];
  INEXACT MT_Float u3, v3;
  MT_Float temp8[8], temp16a[16], temp16b[16], temp16c[16];
  MT_Float temp32a[32], temp32b[32], temp48[48], temp64[64];
  int temp8len, temp16alen, temp16blen, temp16clen;
  int temp32alen, temp32blen, temp48len, temp64len;
  MT_Float axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int axtbblen, axtcclen, aytbblen, aytcclen;
  MT_Float bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int bxtaalen, bxtcclen, bytaalen, bytcclen;
  MT_Float cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int cxtaalen, cxtbblen, cytaalen, cytbblen;
  MT_Float axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  MT_Float axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  MT_Float axtbctt[8], aytbctt[8], bxtcatt[8];
  MT_Float bytcatt[8], cxtabtt[8], cytabtt[8];
  int axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  MT_Float abt[8], bct[8], cat[8];
  int abtlen, bctlen, catlen;
  MT_Float abtt[4], bctt[4], catt[4];
  int abttlen, bcttlen, cattlen;
  INEXACT MT_Float abtt3, bctt3, catt3;
  MT_Float negate;
  ...
  adx = (MT_Float) (pa[0] - pd[0]);
  bdx = (MT_Float) (pb[0] - pd[0]);
  cdx = (MT_Float) (pc[0] - pd[0]);
  ady = (MT_Float) (pa[1] - pd[1]);
  bdy = (MT_Float) (pb[1] - pd[1]);
  cdy = (MT_Float) (pc[1] - pd[1]);
...
MT_Float incircle(point pa, point pb, point pc, point pd)
{
  MT_Float adx, bdx, cdx, ady, bdy, cdy;
  MT_Float bdxcdy, cdxbdy, cdxady, adxcdy, adxbdy, bdxady;
  MT_Float alift, blift, clift;
  MT_Float det;
  MT_Float permanent, errbound;
...
void enqueuebadtri(struct triedge * instri, MT_Float angle, point insapex, point insorg, point insdest)
{
...
void testtriangle(struct triedge *testtri)
{
  ...
  point anglevertex;
  MT_Float dxod, dyod, dxda, dyda, dxao, dyao;
  MT_Float dxod2, dyod2, dxda2, dyda2, dxao2, dyao2;
  MT_Float apexlen, orglen, destlen;
  MT_Float angle;
  MT_Float area;
  shelle sptr;                      /* Temporary variable used by tspivot(). */
  ...
  if ((edge1.sh != dummysh) && (edge2.sh != dummysh)) {
    ...
    if ((angle > 0.9924) && !quiet) {                  /* Roughly 5 degrees. */
      ...
      angle = acos(sqrt(angle)) * (180.0 / TRI_PI);
      printf(
...
enum locateresult preciselocate(point searchpoint, struct triedge *searchtri)
{
  ...
  while (1) {
    ...
    if (searchtri->tri == dummytri) {
      ...
      fdest = swappoint;
      apex(*searchtri, fapex);
      /* Check if the point MT_Floatly is beyond the triangulation boundary. */
      destorient = counterclockwise(forg, fapex, searchpoint);
      orgorient = counterclockwise(fapex, fdest, searchpoint);
...
/*  are converted into MT_Float triangles of the triangulation.  The procedure   */
/*  pulls each hull's bounding triangles apart, then knits them together     */
/*  like the teeth of two gears.  The Delaunay property determines, at each  */
/*  step, whether the next "tooth" is a bounding triangle of the left hull   */
/*  or the right.  When a bounding triangle becomes MT_Float, its apex is        */
/*  changed from NULL to a MT_Float point.                                       */
/*                                                                           */
/*  Only two new triangles need to be allocated.  These become new bounding  */
/*  triangles at the top and bottom of the seam.  They are used to connect   */
/*  the remaining bounding triangles (those that have not been converted     */
/*  into MT_Float triangles) into a single fan.                                  */
/*                                                                           */
...
long divconqdelaunay()
{
  ...
  /* Discard duplicate points, which can MT_Floatly mess up the algorithm. */
  i = 0;
...
MT_Float circletop(point pa, point pb, point pc, MT_Float ccwabc)
{
  MT_Float xac, yac, xbc, ybc, xab, yab;
  MT_Float aclen2, bclen2, ablen2;
...
struct splaynode *circletopinsert(struct splaynode *splayroot, struct triedge *newkey, point pa, point pb, point pc, MT_Float topy)
{
  MT_Float ccwabc;
  MT_Float xac, yac, xbc, ybc;
  MT_Float aclen2, bclen2;
  MT_Float searchpoint[2];
...
long sweeplinedelaunay()
{
  ...
  do {
    if (heapsize == 0)
        throw std::exception( "Error:  Input points are all identical.\n" );
...
int reconstruct(int *trianglelist, MT_Float *triangleattriblist, MT_Float *triangleaMT_Floatist, int elements,int corners, int attribs, int *segmentlist, uint*segmentmarkerlist, uint numberofsegments)
{
  int pointindex;
  int attribindex;
  struct triedge triangleloop;
  ...
  if (poly) {
    ...
    segmentmarkers = segmentmarkerlist != (uint *) NULL;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < firstnumber) || (corner[j] >= firstnumber + inpoints))
          throw std::exception( "Error:  Triangle %d has an invalid vertex index." );
    }
    ...
    if (vararea) {
      area = triangleaMT_Floatist[elementnumber - firstnumber];
    ...
    while (shelleloop.sh != (shelle *) NULL) {
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < firstnumber) || (end[j] >= firstnumber + inpoints))
            throw std::exception( "Error:  Segment %d has an invalid vertex index." );
      }
...
int formskeleton(int *segmentlist, uint *segmentmarkerlist, int numberofsegments)
{
    char polyfilename[6];
    ...
    if (poly) 
    {
        ...
        {
            end1 = segmentlist[indexTemp++];
            end2 = segmentlist[indexTemp++];
            if (segmentmarkers) 
            {
...
void regionplague( MT_Float attribute, MT_Float area)
{
  struct triedge testtri;
  struct triedge neighbor;
  triangle **virusloop;
...
void carveholes(MT_Float *holelist, int holes, MT_Float *regionlist, int regions)
{
  struct triedge searchtri;
  struct triedge triangleloop;
...
void repairencs( int flaws)
{
  ...
  point newpoint;
  enum insertsiteresult success;
  MT_Float segmentlength, nearestpoweroftwo;
  MT_Float split;
...
enum circumcenterresult findcircumcenter(point torg, point tdest, point tapex, point circumcenter, MT_Float* xi,MT_Float* eta)
{
  MT_Float xdo, ydo, xao, yao, xad, yad;
  MT_Float dodist, aodist, addist;
  MT_Float denominator;
  MT_Float dx, dy;
...
void transfernodes(MT_Float *pointlist, MT_Float *pointattriblist, int* pointmarkerlist, int numberofpoints, int numberofpointattribs)
{
  point pointloop;
  MT_Float x, y;
  int i, j;
  ...
  readnodefile = 0;
  if (inpoints < 3)
      throw std::exception("Error:  Input must have at least three input points.");
...
void writenodes(MT_Float **pointlist, MT_Float **pointattriblist, int **pointmarkerlist)
{
  MT_Float *plist;
  MT_Float *palist;
  int *pmlist;
  ...
  if (*pointlist == (MT_Float *) NULL) {
    *pointlist = (MT_Float *) malloc(points.items * 2 * sizeof(MT_Float));
    if (*pointlist == (MT_Float *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
  /* Allocate memory for output point attributes if necessary. */
  if ((nextras > 0) && (*pointattriblist == (MT_Float *) NULL)) {
    *pointattriblist = (MT_Float *) malloc(points.items * nextras * sizeof(MT_Float));
    if (*pointattriblist == (MT_Float *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
...
void writeelements(int **trianglelist, MT_Float** triangleattriblist)
{
  int *tlist;
  MT_Float *talist;
  int pointindex;
  ...
  if ((eextras > 0) && (*triangleattriblist == (MT_Float *) NULL)) {
    *triangleattriblist = (MT_Float *) malloc(triangles.items * eextras *
                                          sizeof(MT_Float));
    if (*triangleattriblist == (MT_Float *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
  tlist = *trianglelist;
...
void writepoly(int **segmentlist, uint **segmentmarkerlist)
{
  int *slist;
  uint *smlist;
  int indexTemp;
  ...
  if (!nobound && (*segmentmarkerlist == (uint *) NULL)) {
    *segmentmarkerlist = (uint *) malloc(shelles.items * sizeof(uint));
    if (*segmentmarkerlist == (uint *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
  slist = *segmentlist;
  ...
  while (shelleloop.sh != (shelle *) NULL) {
    ...
    slist[indexTemp++] = pointmark(endpoint1);
    slist[indexTemp++] = pointmark(endpoint2);
    if (!nobound) {
      /* Copy the boundary marker. */
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if ((triangleloop.tri < trisym.tri) || (trisym.tri == dummytri)) {
        org(triangleloop, p1);
        dest(triangleloop, p2);
        elist[indexTemp++] = pointmark(p1);
        elist[indexTemp++] = pointmark(p2);
        if (nobound) {
        } else {
...
void writevoronoi(MT_Float **vpointlist, MT_Float **vpointattriblist, int **vpointmarkerlist, int **vedgelist,int **vedgemarkerlist, MT_Float **vnormlist)
{
  MT_Float *plist;
  MT_Float *palist;
  int *elist;
  ...
  if (*vpointlist == (MT_Float *) NULL) {
    *vpointlist = (MT_Float *) malloc(triangles.items * 2 * sizeof(MT_Float));
    if (*vpointlist == (MT_Float *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (MT_Float *) NULL) {
    *vpointattriblist = (MT_Float *) malloc(triangles.items * nextras *
                                        sizeof(MT_Float));
    if (*vpointattriblist == (MT_Float *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
  ...
  *vedgemarkerlist = (int *) NULL;
  ...
  if (*vnormlist == (MT_Float *) NULL) {
    *vnormlist = (MT_Float *) malloc(edges * 2 * sizeof(MT_Float));
    if (*vnormlist == (MT_Float *) NULL)
        throw std::exception( "Error:  Out of memory." );
  }
  elist = *vedgelist;
...
void writeneighbors(int **neighborlist)
{
  ...
  * (int *) (dummytri + 6) = -1;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    nlist[indexTemp++] = neighbor1;
    nlist[indexTemp++] = neighbor2;
    nlist[indexTemp++] = neighbor3;
...
void quality_statistics()
{
  struct triedge triangleloop;
  point p[3];
  MT_Float cossquaretable[8];
  MT_Float ratiotable[16];
  MT_Float dx[3], dy[3];
  MT_Float edgelength[3];
  MT_Float dotproduct;
  MT_Float cossquare;
  MT_Float triarea;
  MT_Float shortest, longest;
  MT_Float trilongest2;
  MT_Float smallestarea, biggestarea;
  MT_Float triminaltitude2;
  MT_Float minaltitude;
  MT_Float triaspect2;
  MT_Float worstaspect;
  MT_Float smallestangle, biggestangle;
  MT_Float radconst, degconst;
  int angletable[18];
  int aspecttable[16];
  int aspectindex;
  int tendegree;
  ...
  radconst = TRI_PI / 18.0;
  degconst = 180.0 / TRI_PI;
  for (i = 0; i < 8; i++) {
    cossquaretable[i] = cos(radconst * (MT_Float) (i + 1));
...
{
  MT_Float *holearray;                                        /* Array of holes. */
  MT_Float *regionarray;   /* Array of regional attributes and area constraints. */
#ifndef NO_TIMER
  ...
  if (refine) {
    ...
    hullsize = reconstruct(in->trianglelist, in->triangleattributelist,
                           in->triangleaMT_Floatist, in->numberoftriangles,
  ...
  if (poly || convex) {
    ...
    if (nopolywritten || noiterationnum) {
      ...
      if (poly) {
        ...
        out->holelist = (MT_Float *) NULL;
        out->regionlist = (MT_Float *) NULL;
      }
    }
  }
  if (edgesout) {
