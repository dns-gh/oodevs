...
public:
    ...
    virtual void DrawIcon         ( const char** xpm, const geometry::Point2f& where, float size = -1.f, float factor = 1.f, E_Unit unit = meters ) const = 0;
    ...
    virtual void DrawHQSymbol( const std::string& symbol, const geometry::Point2f& where, float factor, unsigned int direction ) const = 0;
