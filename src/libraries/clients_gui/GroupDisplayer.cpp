...
GroupDisplayer& GroupDisplayer::AddLabel( const QString& name, bool bold )
{
    ...
    if( item )
        throw std::runtime_error( "Item '" + std::string( name.toUtf8().constData() ) + "' already added" );
...
Displayer_ABC& GroupDisplayer::SubItem( const QString& name )
{
    Displayer_ABC* item = items_[ name ];
    if( ! item )
        throw std::runtime_error( "Item '" + std::string( name.toUtf8().constData() ) + "' does not exist" );
    show();
    return *item;
}
