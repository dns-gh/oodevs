...
void QueryDatabaseUpdater::Update( const sword::UnitKnowledgeCreation& msg )
{
    ...
    builder.SetId( "id" );
    builder.SetField( "public_oid", (long)msg.knowledge().id() );
...
void QueryDatabaseUpdater::Update( const sword::ObjectCreation& msg )
{
    ...
    builder.SetId( "id" );
    builder.SetField( "public_oid", (long)msg.object().id() );
    builder.SetField( "name", std::string( msg.name() ) );
    builder.SetField( "type", std::string( msg.type().id() ) );
    builder.SetField( "session_id", session_.GetId() );
    UpdateSymbol( builder, model_.Objects(), msg.object().id() );
...
void QueryDatabaseUpdater::UpdateObjectKnowledgeGeometry( const std::string& tablename, const sword::ObjectKnowledgeUpdate& msg )
{
    ...
    if( ! bHasRows )
    {
        ...
        builderGeom.SetField( "public_oid", (long)msg.knowledge().id() );
...
void QueryDatabaseUpdater::Update( const sword::AutomatCreation& message )
{
    ...
    builder.SetId( "id" );
    builder.SetField( "public_oid", ( long ) message.automat().id() );
    ...
    builder.SetField( "engaged", 0 );
    builder.SetField( "session_id", session_.GetId() );
    UpdateSymbol( builder, model_.Automats(), message.automat().id() );
    database_.Execute( builder );
}
...
void QueryDatabaseUpdater::Update( const sword::AutomatAttributes& msg )
{
    ...
    builder.SetClause( query.str() );
    if( msg.has_mode() )
        builder.SetField( "engaged", ( msg.mode() == sword::engaged ) ? -1 : 0 );
    database_.Execute( builder );
}
