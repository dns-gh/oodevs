...
void Function::AddParameter( boost::shared_ptr< Element_ABC > element )
{
    try
    {
        if( parameterSet_ >= parameters_.size() )
            throw std::exception( tools::translate( "Indicators", "too many parameters." ).toUtf8().constData() );
    ...
    {
        throw std::exception( tools::translate( "Indicators", "Parameter type mismatch in function '%1': %2" ).arg( name_ ).arg( e.what() ).toUtf8().constData() );
    }
}
...
void Function::Serialize( xml::xostream& xos, ElementDeclarator_ABC& declarator ) const
{
    if( parameters_.size() > parameterSet_ )
        throw std::exception( tools::translate( "Indicators", "Parameter type mismatch in function '%1': too few parameters." ).arg( name_ ).toUtf8().constData() );
    ...
    xos << xml::start( category_ )
            << xml::attribute( "function", name_.toUtf8().constData() );
    if( category_ != "result" )
