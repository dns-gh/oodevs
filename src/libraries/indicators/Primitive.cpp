...
// -----------------------------------------------------------------------------
// Name: Primitive::GetShortName
// Created: LDC 2012-10-01
// -----------------------------------------------------------------------------
QString Primitive::GetShortName() const
{
    return GetName();
}
...
QString Primitive::BuildParameterList() const
{
    ...
    BOOST_FOREACH( const T_Parameters::value_type& parameter, parameters_ )
        list.push_back( parameter->GetName().toAscii().constData() ); // $$$$ ABR 2012-08-14: Do NOT use .toStdString() here or it will crash
