...
boost::shared_ptr< Element_ABC > ElementFactory::CreateVariable( const std::string& name ) const
{
    ...
    if( element == 0 )
        throw std::exception( tools::translate( "Indicators", "Undefined variable: %1." ).arg( name.c_str() ).toStdString().c_str() );
    return element;
}
...
boost::shared_ptr< Element_ABC > ElementFactory::CreateFunction( const std::string& name ) const
{
    ...
    throw std::exception( tools::translate( "Indicators", "Undefined function: %1." ).arg( name.c_str() ).toStdString().c_str() );
}
