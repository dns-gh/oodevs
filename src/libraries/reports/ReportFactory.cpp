...
std::string ReportFactory::FormatReport( const sword::Report& message ) const
{
    ReportTemplate* report = Find( message.type().id() );
    if( report )
        return report->RenderMessage( message ).toUtf8().constData();
    return std::string();
}
...
QString ReportFactory::RenderParameter( const sword::MissionParameter_Value& value ) const
{
    ...
    throw std::runtime_error( tools::translate( "ReportFactory", "Unhandled report parameter type: '%1'." ).arg( value.GetDescriptor()->full_name().c_str() ).toUtf8().constData() );
}
