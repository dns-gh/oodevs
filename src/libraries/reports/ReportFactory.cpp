...
// -----------------------------------------------------------------------------
QDateTime ReportFactory::GetTime( const ASN1T_DateTime& d ) const
{
...
// -----------------------------------------------------------------------------
std::string ReportFactory::FormatReport( const ASN1T_MsgReport& asn ) const
{
    ReportTemplate* report = Find( asn.cr );
    if( report )
...
QString ReportFactory::RenderParameter( const ASN1T_MissionParameter& value ) const
{
    ...
    case T_MissionParameter_value_unit:
        return rcResolver_.CreateLink( Agent_ABC::typeName_, value.value.u.unit );
