...
ActionParameterEntityList::ActionParameterEntityList( const kernel::OrderParameter& parameter, const ASN1T_ListAgent& asn, const kernel::Resolver_ABC< kernel::Agent_ABC >& agents )
    : ActionParameter< QString >( parameter.GetName() )
{
    for( unsigned int i = 0; i < asn.n; ++i )
        AddParameter( *new ActionParameter< const kernel::Agent_ABC* >( tools::translate( "ActionParameter", "Agent %1" ).arg( i ), &agents.Get( asn.elem[i] ) ) );
}
...
{
    for( unsigned int i = 0; i < asn.n; ++i )
        AddParameter( *new ActionParameter< const kernel::Automat_ABC* >( tools::translate( "ActionParameter", "Automat %1" ).arg( i ), &automats.Get( asn.elem[i] ) ) );
}
