...
Team_ABC* TeamFactory::CreateTeam( unsigned long id, DIN::DIN_Input& input )
{
    ...
    result->Attach< TacticalHierarchies >( *new TeamHierarchies( controllers_.controller_, *result ) );
...
KnowledgeGroup_ABC* TeamFactory::CreateKnowledgeGroup( unsigned long id, Team_ABC& team  )
{
    ...
    result->Attach< TacticalHierarchies  >( *new KnowledgeGroupHierarchies( controllers_.controller_, team, *result ) ); // $$$$ AGE 2006-09-20: 
    result->Update( InstanciationComplete() );
    return result;
}
