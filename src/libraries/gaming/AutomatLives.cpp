...
// Created: AGE 2006-10-06
// -----------------------------------------------------------------------------
void AutomatLives::Draw( const geometry::Point2f& where,
                         const gui::Viewport_ABC& viewport,
                         gui::GLView_ABC& view ) const
{
    const auto& options = view.GetCurrentOptions();
    if( !automat_.IsAnAggregatedSubordinate() &&
        options.IsAggregated( automat_ ) &&
        viewport.IsHotpointVisible() )
        view.DrawLife( where, GetLife(), options.GetRatio( automat_ ) );
}
