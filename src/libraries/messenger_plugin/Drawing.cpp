...
Drawing::Drawing( unsigned int id, const sword::ShapeCreationRequest& asn, const kernel::CoordinateConverter_ABC& converter )
    ...
    , color_    ( QColor( asn.shape().color().red(), asn.shape().color().green(), asn.shape().color().blue() ).name().toUtf8().constData() )
...
void Drawing::Update( const sword::ShapeUpdateRequest& asn )
{
    ...
    if( asn.has_color() )
        color_ = QColor( asn.color().red(), asn.color().green(), asn.color().blue() ).name().toUtf8().constData();
    if( asn.has_pattern() )
