...
bool UrbanDisplayOptions::SetColor( UrbanColor_ABC& color, float livingSpace, const T_HumansStrMap& humans, const Usages_ABC& usages, unsigned int structuralState )
{
    ...
    else if( accommodationColor_ )
    {
        ...
        {
            float proportion = static_cast< float >( usages.Find( accommodationDisplayed_.toUtf8().constData() ) ) / 100.f;
            const AccommodationType* accommodationType = accommodationTypes_.Find( accommodationDisplayed_.toUtf8().constData() );
