...
{
    root_ = new Level( "b", 0 );
    Register( root_->GetId(), root_->GetName().toUtf8().constData(), *root_ );
...
Level* FormationLevels::AddLevel( Level& root, const QString& name )
{
    ...
    Register( newLevel->GetId(), newLevel->GetName().toUtf8().constData(), *newLevel );
    return newLevel;
}
...
{
    return Find( name.toUtf8().constData() );
}
