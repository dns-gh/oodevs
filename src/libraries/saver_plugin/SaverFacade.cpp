...
void SaverFacade::Receive( const sword::SimToClient& wrapper )
{
    if( wrapper.message().has_control_begin_tick() )
        StartFrame( wrapper, wrapper.message().control_begin_tick() );
...
// -----------------------------------------------------------------------------
void SaverFacade::StartFrame( const sword::SimToClient& message, const sword::ControlBeginTick& controlBeginTickMsg )
{
    if( firstTick_ && config_.HasCheckpoint() )
        firstTick_ = false;
    ...
    if( keyFrameIndex_ == 0 )
        saver_->SaveKeyFrame( ModelMessage( model_, encodingBuffer_, firstTick_ ) );
    saver_->StartFrame( Message( SerializeToString( message, encodingBuffer_ ) ), controlBeginTickMsg );
    ++fragmentIndex_;
    ++keyFrameIndex_;
    firstTick_ = false;
}
