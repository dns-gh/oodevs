...
DECLARE_HOOK( GetAltitudeCost, double, ( const void* handler, const MT_Vector2D& from, const MT_Vector2D& to, double rAltitudeCostPerMeter ) )
DECLARE_HOOK( GetFuseauxCost, double, ( const void* handler, const MT_Vector2D& from, const MT_Vector2D& to,
...
DECLARE_HOOK( GetEnemiesCost, double, ( const void* handler, const MT_Vector2D& from, const MT_Vector2D& to, const TerrainData& nToTerrainType,
              const TerrainData& nLinkTerrainType, double rEnemyMaximumCost ) )
DECLARE_HOOK( GetObjectsCost, double, ( const void* handler, const MT_Vector2D& from, const MT_Vector2D& to, const TerrainData& nToTerrainType, const TerrainData& nLinkTerrainType ) )
DECLARE_HOOK( GetPopulationsCost, double, ( const void* handler, const MT_Vector2D& from, const MT_Vector2D& to,
...
Agent_PathfinderRule::Agent_PathfinderRule( const sword::wrapper::View& entity, Agent_Path& path, const MT_Vector2D& from, const MT_Vector2D& to )
    ...
    , entity_                        ( entity )
    , path_                          ( path )
    , handler_                       ( GET_HOOK( GetPathHandler )( path_.GetID() ) )
...
double Agent_PathfinderRule::GetCost( const MT_Vector2D& from, const MT_Vector2D& to, const TerrainData& nToTerrainType, const TerrainData& nLinkTerrainType ) // $$$$ MCO 2012-05-23: executed in pathfind threads !
{
    ...
    const double rAltitudeCost = GET_HOOK( GetAltitudeCost )( handler_, from, to, rAltitudeCostPerMeter_ );
    if( rAltitudeCost < 0 )
    ...
    // Fuseaux
    const double rFuseauxCost = GET_HOOK( GetFuseauxCost )( handler_, from, to,
    ...
    //urban blocks
    const double rUrbanBlockCost = path_.GetPathClass().IsFlying() ? 0 : GET_HOOK( GetUrbanBlockCost )( handler_, from, to );
    ...
    const double rObjectsCost =
        ...
        ? 0 : GET_HOOK( GetObjectsCost )( handler_, from, to, nToTerrainType, nLinkTerrainType );
    if( rObjectsCost < 0 || rSpeed <= 0. )
        return IMPOSSIBLE_WAY( "Objects" );
    rDynamicCost += rObjectsCost;

    // enemies
    const double rEnemiesCost = GET_HOOK( GetEnemiesCost )( handler_, from, to, nToTerrainType, nLinkTerrainType, rEnemyMaximumCost_ );
    if( rEnemiesCost < 0 )
    ...
    const double rPopulationsCost = path_.GetPathClass().HandlePopulations()
        ? GET_HOOK( GetPopulationsCost )( handler_, from, to, nToTerrainType, nLinkTerrainType, path_.GetPathClass().GetPopulationMaximumCost() )
        : 0;
