...
public:
    ...
    virtual void NotifyClientAuthenticated( ClientPublisher_ABC& client, const std::string& link, Profile_ABC& profile, bool silent )
    {
        for( auto it = plugins_.begin(); it != plugins_.end(); ++it )
            (*it)->NotifyClientAuthenticated( client, link, profile, silent );
    }
