...
void SpawnCommand::Start()
{
                         ...
                         &internal_->pid_) )                    // lpProcessInformation
    {
        ...
        throw std::exception( tools::translate( "SpawnCommand", "Could not start process: %1, error: %2" ).arg( debug.c_str() ).arg( errCode ).toUtf8().constData() );
    }
...
{
    workingDirectory_ = directory.toUtf8().constData();
}
