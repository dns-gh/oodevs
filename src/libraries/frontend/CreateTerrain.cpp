...
namespace
{
    ...
    {
        std::string directory = GetDirectory().toUtf8().constData();
...
{
    const std::string directory = config.GetTerrainDir( name.toUtf8().constData() );
    bfs::create_directories( directory );

    AddArgument( QString( "--out=\"%1\"" ).arg( directory.c_str() ) );
    QString qdirectory = GetDirectory();
    std::string rundirectory = qdirectory.toUtf8().constData();
