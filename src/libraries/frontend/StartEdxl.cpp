...
StartEdxl::StartEdxl( const tools::GeneralConfig& config, const QString& exercise, const QString& session, bool attach )
    ...
    , configManipulator_( new ConfigurationManipulator( config, exercise.toAscii().constData(), session.toAscii().constData() ) )
{
    namespace bfs = boost::filesystem;
    bfs::path iniFile = bfs::path( config.GetExerciseDir( exercise.toAscii().constData() ) ) / "edxl.ini";
