...
namespace
{
    ...
    {
        bfs::path p( config.GetCheckpointsDir( exercise.toUtf8().constData(), session.toUtf8().constData() ) );
        p = p / checkpoint.toUtf8().constData() / "MetaData.xml";
        try
        {
            if( bfs::exists( p ) )
            {
...
{
    if( exercise_ != exercise.toUtf8().constData() || session_ != session.toUtf8().constData() )
    {
        exercise_ = exercise;
        session_ = session;
        list_->clear();
        checkpoints_ = frontend::commands::ListCheckpoints( config_, exercise.toUtf8().constData(), session.toUtf8().constData() );
