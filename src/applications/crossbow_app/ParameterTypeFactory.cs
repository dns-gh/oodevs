...
namespace Sword
{
    namespace Crossbow
    {
        public abstract class ParameterType_ABC
        {
            ...
            {
                IFeatureWorkspace ws = (IFeatureWorkspace)Tools.GetWorkspace(Tools.GetExtension().Config.SharedFile);
        ...
        public class ParameterTypeFactory
        {
            class ParameterTypePoint : ParameterType_ABC
            {
                ...
                {
                    string table = Tools.GetExtension().Config.LayersConfiguration.OrderParameterPoint;
                    SerializeGeometry(table, parameterID, UpdateZAware(m_point));                    
                }
            }
            ...
            class ParameterTypePolygon : ParameterType_ABC
            {
                ...
                public override string Value
                {
                    get
                    {
                        return "OrderParameters_Area";
                    }
                }

                public override void Serialize(int parameterID) 
                {
                    string table = Tools.GetExtension().Config.LayersConfiguration.OrderParameterArea;
                    SerializeGeometry(table, parameterID, (IGeometry)m_polygon);
                }
            }
            ...
            sealed class ParameterTypePath : ParameterType_ABC
            {
                ...
                public override string Value
                {
                    get
                    {
                        return "OrderParameters_Line";
                    }
                }

                public override void Serialize(int parameterID)
                {
                    string table = Tools.GetExtension().Config.LayersConfiguration.OrderParameterLine;
                    SerializeGeometry(table, parameterID, m_path);
                }
            }
            ...
            public static void Initialize(IFeatureWorkspace workspace)
            {
                Tools.ClearClass(workspace, Tools.GetExtension().Config.LayersConfiguration.OrderParameterArea);
                Tools.ClearClass(workspace, Tools.GetExtension().Config.LayersConfiguration.OrderParameterLine);
                Tools.ClearClass(workspace, Tools.GetExtension().Config.LayersConfiguration.OrderParameterPoint);
            }
