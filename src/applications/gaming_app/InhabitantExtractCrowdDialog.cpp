    ...
    , originalInhabitantSize_( 0 )
{
    ...
    grid->addWidget( new QLabel( tools::translate( "InhabitantExtractCrowdDialog", "Remaining inhabitant:" ), this ), 6, 0 );
...
void InhabitantExtractCrowdDialog::Validate()
{
    ...
    {
        actions::Action_ABC* action = actionsModel_.CreateInhabitantChangeHealthStateAction( selected_->GetHealthy() - healthySpinBox_->value(),
                                                                                             selected_->GetWounded() - woundedSpinBox_->value(),
                                                                                             selected_->GetDead(), *selected_, controllers_.controller_, static_.types_ );
        action->Attach( *new actions::ActionTiming( controllers_.controller_, simulation_ ) );
        action->Attach( *new actions::ActionTasker( selected_, false ) );
        actionsModel_.Publish( *action );
    }
    // Throw Crowd_Creation
    {
        const Inhabitant& entity = static_cast< const Inhabitant& > ( *( selected_.ConstCast() ) );
        const kernel::TacticalHierarchies& tacticalHierarchy = entity.Get< kernel::TacticalHierarchies >();
        const kernel::Entity_ABC& top = tacticalHierarchy.GetTop();
        actions::Action_ABC* action = actionsModel_.CreateCrowdCreationAction( entity.GetType().GetCrowdType(), 
                                                                               healthySpinBox_->value() + woundedSpinBox_->value(), 
                                                                               entity.GetPosition(),
                                                                               top, controllers_.controller_, static_.types_, static_.coordinateConverter_ );
        action->Attach( *new actions::ActionTiming( controllers_.controller_, simulation_ ) );
