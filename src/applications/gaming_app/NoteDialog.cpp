...
void NoteDialog::OnAccept()
{
    if (update_)
    {
        ...
        message().set_name( textName_->text().toUtf8().constData() );
        message().mutable_marker()->set_id( noteId_ );
        message().set_number( textId_->text().toUtf8().constData() );
        QString text = textDesc_->text();
        message().set_description( text.toUtf8().constData() );
    ...
    else
    {
        ...
        message().mutable_marker()->set_name( textName_->text().toUtf8().constData() );
        message().mutable_marker()->set_number( textId_->text().toUtf8().constData() );
        QString text = textDesc_->text();
        message().mutable_marker()->set_description( text.toUtf8().constData() );
