	...
	, nLogSupplySuperior_ ( 0 )
{
    ...
    pGtia_ = & App::GetApp().GetModel().GetGtia( asnMsg.oid_groupe_connaissance );
}
	...
	, nLogSupplySuperior_ ( 0 )
{
    ...
    pParent_ = & App::GetApp().GetModel().GetAgent( asnMsg.oid_automate );
...
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgChangeLiensLogistiquesAck& asnMsg )
{
    if( asnMsg.m.oid_tc2Present )
		nTC2_ = asnMsg.oid_tc2;
    if( asnMsg.m.oid_maintenancePresent )
    ...
    if( asnMsg.m.oid_ravitaillementPresent )
        nLogSupplySuperior_ = asnMsg.oid_ravitaillement;
    App::GetApp().NotifyAgentUpdated( *this );
}
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const DetectionMessage& input )
{
    ...
    input >> nMaxVisType;
    input >> bRecordMode;
    Agent* pAgent = & App::GetApp().GetModel().GetAgent( nAgentID );
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const PopulationCollisionMessage& msg )
{
    uint32  nPopulationID;

    msg >> nPopulationID;
    Population* pPopulation = & App::GetApp().GetModel().GetPopulation( nPopulationID );
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ConcentrationDetectionMessage& input )
{
    uint32  nPopulationID;
    uint32  nConcentrationID;
    uint8   nVisType;
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const FlowDetectionMessage& input )
{
    uint32  nPopulationID;
    uint32  nFlowID;
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ObjectDetectionMessage& input )
{
    uint32  nObjectID;
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgLogMaintenanceEtat& asn )
{
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgLogSanteEtat& asn )
{
    if( ! pMedicalData_ )
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgLogRavitaillementEtat& asn )
{
    if( ! pSupplyData_ )
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgLogRavitaillementQuotas& asn )
{
...
{
    OnSuperiorChanged( App::GetApp().GetModel().GetAgent( asnMsg.oid_automate ) ); // $$$$ AGE 2006-02-13: 
    App::GetApp().NotifyAgentReparented( *this );
}
...
{
    OnSuperiorChanged( App::GetApp().GetModel().GetAgent( asnMsg.oid_automate ) );
    App::GetApp().NotifyAgentReparented( *this );
}
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgChangeGroupeConnaissanceAck& asnMsg )
{
    OnSuperiorChanged( App::GetApp().GetModel().GetGtia( asnMsg.oid_groupe_connaissance ) ); // $$$$ AGE 2006-02-13: 
    App::GetApp().NotifyAgentReparented( *this );
}
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const ASN1T_MsgUnitPathFind&              asnMsg )
{
    pathFindVector_.clear();
...
// Created: AGE 2006-02-13
// -----------------------------------------------------------------------------
void Agent::Update( const VisionConesMessage& msg )
{
...
void Agent::ReadODB( InputArchive& archive, bool bAutomata )
{
    ...
    if( bAutomate_ )
    {
        ...
        pGtia_ = & App::GetApp().GetModel().GetGtia( nGtia );
    ...
    else
    {
        ...
        pParent_ = & App::GetApp().GetModel().GetAgent( (MIL_AgentID)nParent );
        pParent_->AddChild( *this );
