...
namespace
{
    struct Aggregator
    {
        Aggregator( bool aggregate ) : aggregate_( aggregate ) {};
        void operator()( Agent& agent ) const
        {
            if( agent.IsAutomate() )
                agent.SetAggregation( aggregate_ );
        };
        bool aggregate_;
    };
};
...
{
    App::GetApp().GetModel().ApplyOnAgents( Aggregator( true ) );
}
...
{
    App::GetApp().GetModel().ApplyOnAgents( Aggregator( false ) );
}
