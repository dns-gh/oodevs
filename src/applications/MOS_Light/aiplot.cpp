...
void   AIPlot::setData( int trace, std::vector< std::pair<double, double> >& valueVector )
{
  int i;
  ...
  if (!zooming) 
  {
    this->qn[trace]=valueVector.size();
    int i = 0;
    for( std::vector< std::pair<double,double> >::iterator itValue = valueVector.begin(); itValue != valueVector.end(); ++itValue )
    {
        this->xtrace[trace][i] = itValue->first;
        this->ytrace[trace][i] = itValue->second;
        ++i;
    }
  }
