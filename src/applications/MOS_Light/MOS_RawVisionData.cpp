...
// $Archive: /MVW_v10/Build/SDK/MOS_Light/src/MOS_RawVisionData.cpp $
// $Author: Age $
// $Modtime: 2/03/05 17:53 $
// $Revision: 34 $
...
void MOS_RawVisionData::InitializeMeteo( MT_InputArchive_ABC& archive )
{
    ...
    if ( !archive.Section( "Ephemeride" ) )
        throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Section 'Meteo::Ephemeride' doesn't exist", archive.RetrieveLastError()->GetInfo() );
    
    std::string strVal;
    if ( !archive.ReadField( "HeureLeverSoleil", strVal ) )
        throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Section 'Meteo::Ephemeride::HeureLeverSoleil' doesn't exist", archive.RetrieveLastError()->GetInfo() );
    ...
    if ( !archive.ReadField( "HeureCoucherSoleil", strVal ) )
        throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Section 'Meteo::Ephemeride::HeureCoucherSoleil' doesn't exist", archive.RetrieveLastError()->GetInfo() );
    ...
    if ( !archive.ReadField( "Lune", strVal ) )
        throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Section 'Meteo::Ephemeride::Lune' doesn't exist", archive.RetrieveLastError()->GetInfo() );
    ...
    if ( !archive.Section( "MeteoGlobale" ) )
        throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Section 'Meteo::MeteoGlobale' doesn't exist", archive.RetrieveLastError()->GetInfo() );
    ...
    else
    {
        while ( archive.NextListElement() )
        {
            if ( !archive.Section( "PatchLocal" ) )
                throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Section 'Meteo::PatchsLocaux::PatchLocal' doesn't exist", archive.RetrieveLastError()->GetInfo() );
            ...
            if ( !archive.ReadAttribute( "hautGauche", strPos ) )
                throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Attribute 'Meteo::PatchsLocaux::PatchLocal::hautGauche' doesn't exist", archive.RetrieveLastError()->GetInfo() );
            MOS_App::GetApp().GetWorld().MosToSimMgrsCoord( strPos, vUpLeft );

            if ( !archive.ReadAttribute( "basDroit", strPos ) )
                throw MT_ScipioException( "PHY_PerceptionManager::InitializeMeteo", __FILE__, __LINE__, "Attribute 'Meteo::PatchsLocaux::PatchLocal::basDroit' doesn't exist", archive.RetrieveLastError()->GetInfo() );
