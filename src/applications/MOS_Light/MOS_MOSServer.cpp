...
// $Archive: /MVW_v10/Build/SDK/MOS_Light/src/MOS_MOSServer.cpp $
// $Author: Nld $
// $Modtime: 17/06/05 22:13 $
// $Revision: 6 $
...
//-----------------------------------------------------------------------------
// Name: MOS_MOSServer::Initialize
// Created: NLD 2002-11-28
//-----------------------------------------------------------------------------
void MOS_MOSServer::Initialize()
{
...
//-----------------------------------------------------------------------------
// Name: MOS_MOSServer::Terminate
// Created: NLD 2002-11-28
//-----------------------------------------------------------------------------
void MOS_MOSServer::Terminate()
{
}
...
{
    const unsigned int nMaxTime = 20 * CLOCKS_PER_SEC / 1000;
    unsigned long nStart = clock();
    do
        dinEngine_.Update();
    while( dinEngine_.GetEventManager().GetPendingEvents() && ( clock() - nStart ) < nMaxTime );
        
    MOS_App::GetApp().GetMainWindow().NotifyNetStatistics( dinEngine_.GetStatistics().GetReceivedBytes(), dinEngine_.GetStatistics().GetSentBytes() );
}
