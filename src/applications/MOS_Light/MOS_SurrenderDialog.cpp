...
// $Archive: /MVW_v10/Build/SDK/MOS_Light/src/MOS_SurrenderDialog.cpp $
// $Author: Nld $
// $Modtime: 14/03/05 11:55 $
// $Revision: 2 $
    ...
    , bInitialized_( false )
{
    ...
    pDataLayout->addWidget( new QLabel( "Camp de prisonnier", this ), 0, 0 );
...
void MOS_SurrenderDialog::SetAgent( const MOS_Agent& agent )
{
    ...
    const MOS_DynaObjectManager::T_DynaObjectMap& objects = MOS_App::GetApp().GetDynaObjectManager().GetObjects();
    for( MOS_DynaObjectManager::CIT_DynaObjectMap itObject = objects.begin(); itObject != objects.end(); ++itObject )
    {
        ...
        pPrisonerCampComboBox_->insertItem( MT_FormatString( "Object %d", object.GetID() ).c_str() );
    }
    bInitialized_ = true;
