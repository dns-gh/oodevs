...
// $Archive: /MVW_v10/Build/SDK/MOS_Light/src/MOS_LogisticSupplyRecompletionDialog.cpp $
// $Author: Sbo $
// $Modtime: 29/07/05 14:54 $
// $Revision: 2 $
...
*/
    ...
    connect( pMunitionsFamilyTable_, SIGNAL( valueChanged( int, int ) ), SLOT( OnDotationChanged( int, int ) ) );
...
void MOS_LogisticSupplyRecompletionDialog::Validate()
{
    ...
    {   
        ...
        for( int nRow = 0; nRow < pDotationsTable_->numRows(); ++nRow )
        {
            ...
            asnDotation.famille_dotation = (ASN1T_EnumFamilleDotation)MOS_App::GetApp().GetRessourceID( pDotationItem->text().ascii() );
    ...
    {   
        ...
        for( int nRow = 0; nRow < pMunitionsFamilyTable_->numRows(); ++nRow )
        {
            ...
            asnMunition.famille_munition = (ASN1T_EnumFamilleMunition)MOS_App::GetApp().GetRessourceID( pMunitionItem->text().ascii() );
