...
{
    return tools::translate( "FilterOrbatReIndexer", "Import an order of battle" ).toUtf8().constData();
}
...
{
    return tools::translate( "FilterOrbatReIndexer", "Import an order of battle into current exercise.<br>May import the populations and the objects if relative checkbox are checked." ).toUtf8().constData();
}
...
{
    return filename_ && filename_->text().toUtf8().constData() && !filename_->text().isEmpty() && boost::filesystem::exists( filename_->text().toUtf8().constData() );
}
...
void FilterOrbatReIndexer::OnBrowse()
{
    ...
    emit( statusChanged( IsValid() && partiesListView_->ParseOrbatFile( filename_->text().toUtf8().constData() ) ) );
}
...
void FilterOrbatReIndexer::Execute()
{
    ...
    xml::xifstream xis( filename_->text().toUtf8().constData() );
