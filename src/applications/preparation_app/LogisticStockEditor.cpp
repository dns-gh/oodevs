    ...
    if( showStocks_ )
    {
        ...
        for( int i = 0; i < eNbrStockCategory; ++i )
            if( dataModel_->item( i )->checkState() == Qt::Checked )
                SupplyLogisticBaseStocks( pLogHierarchy->GetEntity(), static_cast< E_StockCategory >( i ), requirements );
...
// -----------------------------------------------------------------------------
void LogisticStockEditor::FillSupplyRequirements( const Entity_ABC& entity, const E_StockCategory logType, T_Requirements& requirements )
{
...
// Created: MMC 2011-08-31
// -----------------------------------------------------------------------------
void LogisticStockEditor::SupplyLogisticBaseStocks( const Entity_ABC& blLogBase, const E_StockCategory logType, T_Requirements& requirements )
{
...
namespace
{
    E_StockCategory GetDotationLogisticType( const DotationType& dotationType )
    {
...
// Created: MMC 2011-08-10
// -----------------------------------------------------------------------------
void LogisticStockEditor::ComputeRequirements( const Agent_ABC& agent, const E_StockCategory logType, T_Requirements& requirements )
{
...
void LogisticStockEditor::SupplyStocks( std::set< const Agent_ABC* >& entStocks, const T_Requirements& requirements )
{
    for( CIT_Requirements itRequired = requirements.begin(); itRequired != requirements.end(); ++itRequired )
    {
        ...
        int index = static_cast< int >( GetDotationLogisticType( dotationType ) );
        assert( index >= 0 && index < eNbrStockCategory );
        if( dataModel_->item( index )->checkState() == Qt::Checked )
        {
            int days = dataModel_->item( index, 1 )->data( Qt::EditRole ).asInt();
    ...
    while( itLogChildren.HasMoreElements() )
    {
        ...
        if( EntityHelpers::IsLogisticBase( logChildren ) )
        {
            ...
            if( pLogChildrenHierarchy )
            {
                ...
                for( int i = 0; i < eNbrStockCategory; ++i )
                    if( dataModel_->item( i )->checkState() == Qt::Checked )
                        SupplyLogisticBaseStocks( pLogChildrenHierarchy->GetEntity(), static_cast< E_StockCategory >( i ), requirements );
