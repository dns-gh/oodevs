...
void SideList::Update( const QString& exercise )
{
    ...
    try
    {
        if( !exercise.isEmpty() )
        {
            std::auto_ptr< xml::xistream > xis = fileLoader_.LoadFile( config_.GetExerciseFile( exercise.toUtf8().constData() ) );
            std::string orbatFile;
            *xis >> xml::start( "exercise" )
                    >> xml::start( "orbat" )
                        >> xml::attribute( "file", orbatFile );
            UpdateSides( config_.BuildChildPath( config_.GetExerciseFile( exercise.toUtf8().constData() ), orbatFile ) );
        }
    }
    catch(...)
    {
