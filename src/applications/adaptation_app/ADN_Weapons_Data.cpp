...
void ADN_Weapons_Data::WeaponInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( !pLauncher )
        throw ADN_DataException( tools::translate( "Weapons_Data", "Invalid data" ).toAscii().constData(), tools::translate( "Weapons_Data", "Weapon systems '%1'/'%2' - Invalid launcher type" ).arg( strLauncher.c_str(), strAmmunition.c_str() ).toAscii().constData() );
    ...
    if( !pAmmo )
        throw ADN_DataException( tools::translate( "Weapons_Data", "Invalid data" ).toAscii().constData(), tools::translate( "Weapons_Data", "Weapon systems '%1'/'%2' - Invalid ammunition type" ).arg( strLauncher.c_str(), strAmmunition.c_str() ).toAscii().constData() );
...
void ADN_Weapons_Data::WeaponInfos::WriteArchive( xml::xostream& output )
{
    ...
    if( bIndirect_.GetData() )
    {
        if( rMaxRange_.GetData() < rMinRange_.GetData() )
            throw ADN_DataException( tools::translate( "Weapons_Data", "Invalid data" ).toAscii().constData(), tools::translate( "Weapons_Data", "Weapon %1 - Indirect fire - max range < min range" ).arg( strName_.GetData().c_str() ).toAscii().constData() );
