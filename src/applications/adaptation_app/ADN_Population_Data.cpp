...
void ADN_Population_Data::PopulationInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( !pModel )
        throw ADN_DataException( "Invalid data", tools::translate( "Population_Data", "Crowd types - Invalid behavior model '%1'" ).arg( strModel.c_str() ).toUtf8().constData() );
    ptrModel_ = pModel;
...
void ADN_Population_Data::PopulationInfos::ReadSlowingEffect( xml::xistream& input )
{
    ...
    if( nAttitude == (E_PopulationAttitude)-1 )
        throw ADN_DataException( "Invalid data",tools::translate( "Population_Data",  "Crowd types - Invalid crowd attitude '%1'" ).arg( strAttitude.c_str() ).toUtf8().constData() );
...
void ADN_Population_Data::PopulationInfos::ReadFireEffect( xml::xistream& input )
{
    ...
    if( nROE == (E_PopulationRoe)-1 )
        throw ADN_DataException( "Invalid data", tools::translate( "Population_Data", "Crowd types - Invalid crowd 'rules of engagment' '%1'" ).arg( strROE.c_str() ).toUtf8().constData() );
