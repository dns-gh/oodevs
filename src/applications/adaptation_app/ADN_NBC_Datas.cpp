    ...
    if( pWound )
    {
        *pWound = input.attribute< double >( "percentage" ) * 100.;
        if( pWound->GetData() < 0. || pWound->GetData() > 100. )
            throw ADN_DataException( tools::translate( "NBC_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "NBC_Data", "NBC - Wound '%1' data < 0 or > 1" ).arg( wound.c_str() ).toUtf8().constData() );
    }
    else
        throw ADN_DataException( tools::translate( "NBC_Data", "Invalid data" ).toUtf8().constData(),tools::translate( "NBC_Data", "NBC - Invalid wound type '%1'" ).arg( wound.c_str() ).toUtf8().constData() );
}
...
void ADN_NBC_Datas::NbcIntoxInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( bIntoxPresent_.GetData() )
    {
        ...
        if( rNbAlivedHumans_.GetData() + rNbHurtedHumans1_.GetData() + rNbHurtedHumans2_.GetData() + rNbHurtedHumans3_.GetData() + rNbHurtedHumansE_.GetData() + rNbDeadHumans_.GetData() != 100.0 )
            throw ADN_DataException( tools::translate( "NBC_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "NBC_Data","NBC - Agent '%1' - Poisoning effect data sum < 100" ).arg( GetParentNode()->GetNodeName().c_str() ).toUtf8().constData() );
    }
