...
{
    return qApp->translate( "ADN_SaveFile_Exception", "File error" ).toUtf8().constData();
}
...
{
    return qApp->translate( "ADN_SaveFile_Exception", "Could not save file '%1'.\nMake sure that the file is not write-protected." ).arg( strFileName_.c_str() ).toUtf8().constData();
}
