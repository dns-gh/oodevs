...
void ADN_AiEngine_Data::ReadArchive( xml::xistream& input )
{
    ...
    if( rPertinenceMaxDecrease_.GetData() < 0.0 || rPertinenceMaxDecrease_.GetData() > 100.0 )
        throw ADN_DataException( tools::translate( "AiEngine_Data",  "Invalid data" ).toAscii().constData(), tools::translate( "AiEngine_Data", "Op. Indicators - Thread management - Invalid relevance effect value(must be >=0 and <= 100)" ).toAscii().constData() );

    if( rOperationalStateMaxDecrease_.GetData() < 0.0 || rOperationalStateMaxDecrease_.GetData() > 100.0 )
        throw ADN_DataException( tools::translate( "AiEngine_Data","Invalid data" ).toAscii().constData(), tools::translate( "AiEngine_Data", "Op. Indicators - Thread management - Invalid operational state effect value (must be >=0 and <= 100)" ).toAscii().constData() );

    if( rNeutralizedStateMaxDecrease_.GetData() < 0.0 || rNeutralizedStateMaxDecrease_.GetData() > 100.0 )
        throw ADN_DataException( tools::translate( "AiEngine_Data", "Invalid data" ).toAscii().constData(), tools::translate( "AiEngine_Data", "Op. Indicators - Thread management - Invalid neutralization state effect value (must be >=0 and <= 100)" ).toAscii().constData() );
    ...
    if( rMinorEquipmentWeight_.GetData() < 0.f || rMinorEquipmentWeight_.GetData() > 1.f )
        throw ADN_DataException( tools::translate( "AiEngine_Data", "Invalid data" ).toAscii().constData(), tools::translate( "AiEngine_Data", "Op. Indicators - Op. state computation - Invalid equipment weight sum (must be >=0 and <= 1)" ).toAscii().constData() );
    ...
    if( rDecisionalThreshold_.GetData() < 0.f || rDecisionalThreshold_.GetData() > 1.f )
        throw ADN_DataException( tools::translate( "AiEngine_Data", "Invalid data" ).toAscii().constData(), tools::translate( "AiEngine_Data", "Op. Indicators - Op. state computation - Invalid threshold (must be >=0 and <= 1)" ).toAscii().constData() );

    if( rHumanWeight_.GetData() < 0.f || rHumanWeight_.GetData() > 1.f )
        throw ADN_DataException( tools::translate( "AiEngine_Data", "Invalid data" ).toAscii().constData(), tools::translate( "AiEngine_Data", "Op. Indicators - Op. state computation - Invalid crew state weight (must be >=0 and <= 1)" ).toAscii().constData() );
