...
void ADN_Equipement_Data::CategoryInfo::ReadArchive( xml::xistream& input )
{
    ...
    if( !pNature )
        throw ADN_DataException( tools::translate( "Equipment_Data", "Invalid data" ).toAscii().constData(), tools::translate( "Equipment_Data", "Equipment - Invalid resource nature '%1'" ).arg( dotationNature.c_str() ).toAscii().constData() );
...
void ADN_Equipement_Data::IndirectAmmoInfos::ReadPh( xml::xistream& input )
{
    ...
    throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Equipment - Invalid stance '%1'" ).arg( posture.c_str() ).toAscii().constData() );
}
...
void ADN_Equipement_Data::IndirectAmmoInfos::Initialize()
{
    ...
    if( !strObjectType_.empty() )
    {
        ...
        if( !pObject )
        {
            QMessageBox::StandardButton result = QMessageBox::warning( 0, tools::translate( "Equipment_Data", "Invalid data" ), tools::translate( "Equipment_Data", "Invalid object '%1'. Do you wish to continue with a default object instead ?" ).arg( strObjectType_.c_str() ), QMessageBox::Yes | QMessageBox::No );
            if( result == QMessageBox::Yes )
            {
                ...
                throw ADN_DataException( tools::translate( "Equipment_Data", "Invalid data" ).toAscii().constData(), tools::translate( "Equipment_Data", "Invalid object '%1'" ).arg( strObjectType_.c_str() ).toAscii().constData() );
        }
        else
            objectType_ = pObject;
    }
...
void ADN_Equipement_Data::AmmoCategoryInfo::ReadUrbanModifer( xml::xistream& input )
{
    ...
    if( it == modifUrbanBlocks_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Equipment - Invalid urban Material type '%1'" ).arg( material.c_str() ).toAscii().constData() );
...
void ADN_Equipement_Data::AmmoCategoryInfo::ReadArchive( xml::xistream& input )
{
    ...
    if( !type.empty() )
    {
        ...
        if( nType_ == E_MunitionType( -1 ) )
            throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Equipment - Invalid resource type '%1'" ).arg( type.c_str() ).toAscii().constData() );
    }
...
void ADN_Equipement_Data::ReadResource( xml::xistream& input )
{
    ...
    if( nResourceType == -1 )
        throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Equipment - Invalid resource type '%1'" ).arg( category.c_str() ).toAscii().constData() );
