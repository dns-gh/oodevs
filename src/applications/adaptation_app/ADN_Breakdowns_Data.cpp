...
void ADN_Breakdowns_Data::RepairPartInfo::ReadArchive( xml::xistream& input )
{
    ...
    if( pCategory == 0 )
        throw ADN_DataException( tools::translate( "Breakdown_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "Breakdown_Data", "Breakdowns - Invalid part '%1'" ).arg( strCategoryName.c_str() ).toUtf8().constData() );
    ptrPart_ = pCategory;
}
...
ADN_Breakdowns_Data::BreakdownInfo* ADN_Breakdowns_Data::BreakdownInfo::CreateCopy()
{
    ...
    pCopy->strName_ = tools::translate( "Breakdown_Data", "New breakdown" ).toUtf8().constData();
...
void ADN_Breakdowns_Data::BreakdownInfo::ReadArchive( xml::xistream& input )
{
    ...
    if( nType_ == E_BreakdownType( -1 ) )
        throw ADN_DataException( tools::translate( "Breakdown_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "Breakdown_Data", "Breakdowns - Invalid breakdown type '%1'" ).arg( type.c_str() ).toUtf8().constData() );
