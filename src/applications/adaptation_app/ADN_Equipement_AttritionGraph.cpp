...
void ADN_Equipement_AttritionGraph::Update()
{
    ...
    for( IT_Attritions it = attritions_.begin(); it != attritions_.end(); ++it )
        if( (*it)->ptrArmor_.GetData()->strName_.GetData() == info->strName_.GetData() )
        {
            ...
            visuNoEvac.strName_ = ADN_Tr::ConvertFromEquipmentState( eEquipmentState_ADN_FixableInPlace, ENT_Tr_ABC::eToTr ).c_str();
            GraphData visuWithEvac( 6 );
            visuWithEvac.strName_ = ADN_Tr::ConvertFromEquipmentState( eEquipmentState_ADN_FixableWithEvac, ENT_Tr_ABC::eToTr ).c_str();
            GraphData visuDestroyed( 6 );
            visuDestroyed.strName_ = ADN_Tr::ConvertFromEquipmentState( eEquipmentState_ADN_Destroyed, ENT_Tr_ABC::eToTr ).c_str();
