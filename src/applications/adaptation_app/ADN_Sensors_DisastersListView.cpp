...
namespace
{
    ...
    public:
        ...
        ADN_ListViewItem* CreateItem( void* pObj )
        {
            ...
            pItem->Connect( &static_cast< ADN_Radars_Data::DisasterInfos* >( pObj )->ptrDisaster_.GetData()->strName_ );
            return pItem;
        }
        //@}
    };
...
void ADN_Sensors_DisastersListView::ConnectItem( bool bConnect )
{
    ...
    ADN_Radars_Data::DisasterInfos* pInfos = (ADN_Radars_Data::DisasterInfos*)pCurData_;
    ADN_Tools::CheckConnectorVector( vItemConnectors_, ADN_Radars_GUI::eNbrDisasterElements );

    vItemConnectors_[ADN_Radars_GUI::eDetectionThreshold]->Connect( &pInfos->rDetectionThreshold_, bConnect );
}
...
void ADN_Sensors_DisastersListView::OnContextMenu( const QPoint& pt )
{
    ...
    if( nMenuResult == 1 )
    {
        ADN_Radars_Data::DisasterInfos* pCurObject = (ADN_Radars_Data::DisasterInfos*)pCurData_;
        static_cast< ADN_Connector_Vector_ABC* >( pConnector_ )->RemItem( pCurObject );
    }
    else if( nMenuResult > 1 )
    {
        // Add the weapon to the list.
        ADN_Radars_Data::DisasterInfos* pNewInfo = new ADN_Radars_Data::DisasterInfos();
...
bool ADN_Sensors_DisastersListView::Contains( const ADN_Disasters_Data::DisasterInfos* pInfo )
{
    ...
    for( int row = 0; row < rowCount; ++row )
    {
        ...
        ADN_Radars_Data::DisasterInfos* pData = static_cast< ADN_Radars_Data::DisasterInfos* >( pCurr->GetData() );
