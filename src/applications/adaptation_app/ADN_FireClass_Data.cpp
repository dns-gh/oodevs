    ...
    if( pWound )
    {
        *pWound = static_cast< int >( input.attribute< double >( "percentage" ) * 100. );
        if( pWound->GetData() < 0 || pWound->GetData() > 100 )
            throw ADN_DataException( tools::translate( "ADN_FireClass_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "ADN_FireClass_Data", "Fire - Wound '%1' data < 0 or > 1" ).arg( wound.c_str() ).toUtf8().constData() );
    }
    else
        throw ADN_DataException( tools::translate( "ADN_FireClass_Data", "Invalid data" ).toUtf8().constData(),tools::translate( "ADN_FireClass_Data", "Fire - Invalid wound type '%1'" ).arg( wound.c_str() ).toUtf8().constData() );
}
...
void ADN_FireClass_Data::FireInjuryInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( nNbHurtHumans1_.GetData() + nNbHurtHumans2_.GetData() + nNbHurtHumans3_.GetData() + nNbHurtHumansE_.GetData() + nNbDeadHumans_.GetData() > 100 )
        throw ADN_DataException( tools::translate( "ADN_FireClass_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "ADN_FireClass_Data", "Fire '%1' - Injuries data sum > 100" ).arg( GetParentNode()->GetNodeName().c_str() ).toUtf8().constData() );
}
...
void ADN_FireClass_Data::FireClassInfos::ReadAgent( xml::xistream& input )
{
    ...
    if( itAgent == agents_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toUtf8().constData(), tr( "Fire class - Invalid extinguisher agent '%1'" ).arg( agent.c_str() ).toUtf8().constData() );
...
void ADN_FireClass_Data::FireClassInfos::ReadUrbanModifer( xml::xistream& input )
{
    ...
    if( it == modifUrbanBlocks_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toUtf8().constData(), tr( "Fire class - Invalid urban Material type '%1'" ).arg( material.c_str() ).toUtf8().constData() );
...
void ADN_FireClass_Data::FireClassInfos::ReadSurface( xml::xistream& input )
{
    ...
    if( location == static_cast< E_Location >( -1 ) )
        throw ADN_DataException( tr( "Invalid data" ).toUtf8().constData(), tr( "Fire - Invalid location type '%1'" ).arg( type.c_str() ).toUtf8().constData() );*/
    ...
    if( it == surfaceInfos_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toUtf8().constData(), tr( "Fire - Location type not found '%1'" ).arg( type.c_str() ).toUtf8().constData() );
    ( *it )->ReadArchive( input );
}
