    ...
    if( pWound )
    {
        *pWound = static_cast< int >( input.attribute< double >( "percentage" ) * 100. );
        if( pWound->GetData() < 0 || pWound->GetData() > 100 )
            throw ADN_DataException( tools::translate( "ADN_FireClass_Data", "Invalid data" ).toAscii().constData(), tools::translate( "ADN_FireClass_Data", "Fire - Wound '%1' data < 0 or > 1" ).arg( wound.c_str() ).toAscii().constData() );
    }
    else
        throw ADN_DataException( tools::translate( "ADN_FireClass_Data", "Invalid data" ).toAscii().constData(),tools::translate( "ADN_FireClass_Data", "Fire - Invalid wound type '%1'" ).arg( wound.c_str() ).toAscii().constData() );
}
...
void ADN_FireClass_Data::FireClassInfos::ReadAgent( xml::xistream& input )
{
    ...
    if( itAgent == agents_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Fire class - Invalid extinguisher agent '%1'" ).arg( agent.c_str() ).toAscii().constData() );
...
void ADN_FireClass_Data::FireClassInfos::ReadUrbanModifer( xml::xistream& input )
{
    ...
    if( it == modifUrbanBlocks_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Fire class - Invalid urban Material type '%1'" ).arg( material.c_str() ).toAscii().constData() );
...
void ADN_FireClass_Data::FireClassInfos::ReadSurface( xml::xistream& input )
{
    ...
    if( it == surfaceInfos_.end() )
        throw ADN_DataException( tr( "Invalid data" ).toAscii().constData(), tr( "Fire - Location type not found '%1'" ).arg( type.c_str() ).toAscii().constData() );
    ( *it )->ReadArchive( input );
}
