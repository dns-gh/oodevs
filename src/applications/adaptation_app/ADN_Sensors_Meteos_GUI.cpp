...
{
    ADN_Sensors_Data::ModificatorMeteoInfos* pInfos = static_cast< ADN_Sensors_Data::ModificatorMeteoInfos* >( data );
    if( !pInfos )
        return;

    AddItem( row, 0, data, ADN_Tr::ConvertFromSensorWeatherModifiers( pInfos->eType_, ENT_Tr_ABC::eToTr ).c_str(), Qt::ItemIsSelectable );
