...
void ADN_Units_Data::StockLogThresholdInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( eCategory == (E_StockCategory)-1 )
        throw ADN_DataException( tools::translate( "Units_Data", "Invalid data" ).toAscii().constData(), tools::translate( "Units_Data", "Unit types - Invalid resource '%1'" ).arg( strCategory.c_str() ).toAscii().constData() );
    eCategory_ = eCategory;
}
