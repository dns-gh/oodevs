...
void ADN_Units_Data::StockLogThresholdInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( eCategory == (E_StockCategory)-1 )
        throw ADN_DataException( tools::translate( "Units_Data", "Invalid data" ).toStdString(), tools::translate( "Units_Data", "Unit types - Invalid resource '%1'" ).arg( strCategory.c_str() ).toStdString() );
    eCategory_ = eCategory;
}
