...
std::string ADN_Sensors_Data::ModificatorUrbanBlockInfos::GetNodeName()
{
    ...
    return ( QString( strResult.c_str() ) + QString( ptrMaterial_.GetData()->strName_.GetData().c_str() ) ).toUtf8().constData();
}
...
void ADN_Sensors_Data::TargetInfos::ReadPosture( xml::xistream& input )
{
    ...
    throw ADN_DataException( tools::translate( "Sensor_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "Sensor_Data", "Sensors - Invalid stance '%1'" ).arg( type.c_str() ).toUtf8().constData() );
}
...
void ADN_Sensors_Data::TargetInfos::ReadArchive( xml::xistream& input )
{
    ...
    if( !pObject )
        throw ADN_DataException( tools::translate( "Sensor_Data", "Invalid data" ).toUtf8().constData(),tools::translate( "Sensor_Data", "Sensors - Invalid object '%1'" ).arg( strType.c_str() ).toUtf8().constData() );
    ptrObject_ = pObject;
...
void ADN_Sensors_Data::SensorInfos::ReadSize( xml::xistream& input )
{
    ...
    if( it == vModifSizes_.end() )
        throw ADN_DataException( tools::translate( "Sensor_Data", "Invalid data" ).toUtf8().constData(), tools::translate( "Sensor_Data", "Sensors - Invalid unit volume '%1'" ).arg( type.c_str() ).toUtf8().constData() );
...
void ADN_Sensors_Data::SensorInfos::ReadPrecipitation( xml::xistream& input )
{
    ...
    throw ADN_DataException( tools::translate( "Sensor_Data", "Invalid data" ).toUtf8().constData(),tools::translate( "Sensor_Data", "Sensors - Invalid weather '%1'" ).arg( type.c_str() ).toUtf8().constData() );
}
