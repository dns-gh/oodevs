...
void ADN_ListView_Orders::OnContextMenu( const QPoint& pt )
{
    ...
    unsigned int n = 0;
    ADN_Missions_Data::T_Mission_ABC_Vector& fragOrders = ADN_Workspace::GetWorkspace().GetMissions().GetData().GetFragOrders();
    for( auto it = fragOrders.begin(); it != fragOrders.end(); ++it )
    {
        ADN_Missions_Data::FragOrder* fragOrder = static_cast< ADN_Missions_Data::FragOrder* >( *it );
