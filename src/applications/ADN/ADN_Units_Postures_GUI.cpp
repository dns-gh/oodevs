...
public:
    ...
    void AddSubItems( int n, void *pObj )
    {
        ...
        ADN_TableItem_Double* pItemDoubleTimeToActivate = new ADN_TableItem_Double( &tab_, pObj );

        // Add a new row.
        tab_.setItem( n, 0, pItemString );
        tab_.setItem( n, 1, pItemDoubleTimeToActivate );
        pItemDoubleTimeToActivate->GetValidator().setBottom( 0 );
        ...
        if( pInfo->nPosture_ < ePostureNeedTimeStart )
            ...
            pItemDoubleTimeToActivate->GetConnector().Connect( &pInfo->rTimeToActivate_ );
    }
...
: ADN_Table2( pParent, "ADN_Units_Postures_GUI" )
{
    ...
    horizontalHeader()->setLabel(1, tr( "Time to activate (s)"));
