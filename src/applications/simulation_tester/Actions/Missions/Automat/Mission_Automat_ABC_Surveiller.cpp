// *****************************************************************************
//
// This file is part of a MASA library or program.
// Refer to the included end-user license agreement for restrictions.
//
// Copyright (c) 2005 Mathématiques Appliquées SA (MASA)
//
// *****************************************************************************

#include "simulation_tester_pch.h"
#include "Mission_Automat_ABC_Surveiller.h"
#include "Entities/Automat.h"
#include "Messages/ASN_Messages.h"
#include "Tools/ASN_Tools.h"

using namespace TEST;

// -----------------------------------------------------------------------------
// Name: Mission_Automat_ABC_Surveiller constructor
// Created: SBO 2005-08-04
// -----------------------------------------------------------------------------
Mission_Automat_ABC_Surveiller::Mission_Automat_ABC_Surveiller( Automat& Automat )
    : Mission_Automat_ABC ( "Automat_ABC_Surveiller", Automat )
{
    // NOTHING
}

// -----------------------------------------------------------------------------
// Name: Mission_Automat_ABC_Surveiller destructor
// Created: SBO 2005-08-04
// -----------------------------------------------------------------------------
Mission_Automat_ABC_Surveiller::~Mission_Automat_ABC_Surveiller()
{
    // NOTHING
}

// -----------------------------------------------------------------------------
// Name: Mission_Automat_ABC_Surveiller::Serialize
// Created: SBO 2005-08-04
// -----------------------------------------------------------------------------
void Mission_Automat_ABC_Surveiller::Serialize()
{
    // build din/asn msg
    Mission_Automat_ABC::Serialize();

    Mission_Automate_ABC_Surveiller& asnMission = *new Mission_Automate_ABC_Surveiller();

    asnMsg_.GetAsnMsg().mission().mission_automate_abc_surveiller = &asnMission;

    const Location& zoneInstallation_ = pTarget_->GetTestParam_Polygon();

    ASN_Tools::CopyPolygon( zoneInstallation_, asnMission.zone_installation );

    delete &zoneInstallation_;

}

// -----------------------------------------------------------------------------
// Name: Mission_Automat_ABC_Surveiller::Clean
// Created: SBO 2005-08-08
// -----------------------------------------------------------------------------
void Mission_Automat_ABC_Surveiller::Clean()
{
    assert( asnMsg_.GetAsnMsg().mission.has_mission_automate_abc_surveiller ());
    Mission_Automate_ABC_Surveiller& asnMission = *asnMsg_.GetAsnMsg().mission().mission_automate_abc_surveiller;

    ASN_Tools::Delete( asnMission.zone_installation );

    delete &asnMission;
    Mission_Automat_ABC::Clean();
}




