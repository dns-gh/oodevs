...
BOOST_FIXTURE_TEST_CASE( authentication_response_to_client_is_converted, ContextFixture< sword::AuthenticationToClient > )
{
    ...
    FillProfileDescription( content.mutable_authentication_response()->mutable_profiles()->add_elem() );
    content.mutable_authentication_response()->mutable_server_version()->set_value( "1.2.3" );
    ...
    MOCK_EXPECT( client, SendAuthenticationToClient ).once().with( constraint( msg, "context: 42 message { authentication_response { error_code: invalid_login profile { login: \"login\" password: \"password\" read_only_formations { elem { id: 3 } elem { id: 5 } } read_write_formations { elem { id: 6 } elem { id: 7 } } read_only_automates { elem { id: 8 } elem { id: 9 } } read_write_automates { elem { id: 10 } elem { id: 11 } } read_only_camps { elem { id: 12 } elem { id: 13 } } read_write_camps { elem { id: 14 } elem { id: 15 } } read_only_crowds { elem { id: 16 } elem { id: 17 } } read_write_crowds { elem { id: 18 } elem { id: 19 } } supervisor: true role { id: 77 } } profiles { elem { login: \"login\" password: true supervisor: true role { id: 77 } } elem { login: \"login\" password: true supervisor: true role { id: 77 } } } server_version { value: \"1.2.3\" } restart_date_time { data: \"data\" } terrain_name: \"terrain_name\" } }" ) );
    converter.ReceiveAuthenticationToClient( "unused endpoint", msg );
}
