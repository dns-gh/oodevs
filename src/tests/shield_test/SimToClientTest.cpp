...
namespace
{
    ...
    void FillObjectAttributes( A* a )
    {
        ...
        FillLocatedQuantity( a->mutable_toxic_cloud()->mutable_quantities()->add_elem() );
        a->mutable_fire()->set_class_id( 33 );
        a->mutable_fire()->set_heat( 34 );
...
BOOST_FIXTURE_TEST_CASE( object_knowledge_update_to_client_is_converted, ContextFixture< sword::SimToClient > )
{
    ...
    MOCK_EXPECT( client, SendSimToClient ).once().with( constraint( msg, "context: 42 message { object_knowledge_update { knowledge { id: 7 } party { id: 8 } object { id: 9 } relevance: 10 location { type: rectangle coordinates { elem { latitude: 17.23 longitude: 23.17 } elem { latitude: 17.23 longitude: 23.17 } } } attributes { construction { resource { id: 10 } dotation_nbr: 11 density: 12 percentage: 13 } obstacle { type: reserved activated: true activation_time: 14 } mine { resource { id: 15 } dotation_nbr: 16 density: 17 percentage: 18 } activity_time { value: 19 } bypass { percentage: 20 } logistic { tc2 { id: 21 } } nbc { danger_level: 22 nbc_agents { id: 23 } nbc_agents { id: 24 } } crossing_site { width: 25 depth: 26 flow_rate: 27 banks_require_fitting: true } supply_route { equipped: true max_weight: 28 width: 29 length: 30 flow_rate: 31 } toxic_cloud { quantities { elem { coordinate { latitude: 17.23 longitude: 23.17 } quantity: 32 } elem { coordinate { latitude: 17.23 longitude: 23.17 } quantity: 32 } } } fire { class_id: 33 heat: 34 } medical_treatment { external_reference_id: \"ext\" bed_capacities { type_id: 35 baseline_count: 36 available_count: 37 emergency_count: 38 } bed_capacities { type_id: 35 baseline_count: 36 available_count: 37 emergency_count: 38 } facility_status: on_divert doctors: 39 available_doctors: 40 } interaction_height { height: 41 } stock { resources { resource { id: 42 } current: 43 maximum: 44 } resources { resource { id: 42 } current: 43 maximum: 44 } } nbc_agent { agent { id: 45 } concentration: 46 source_life_duration: 47 } effect_delay { value: 48 } seal_off { level: 49 } resource_networks { network { resource { name: \"name\" } link { kind: urban target_id: 50 capacity: 51 flow: 52 } link { kind: urban target_id: 50 capacity: 51 flow: 52 } enabled: true max_stock: 53 stock: 54 production: 55 consumption: 56 critical: true initial_stock: 57 max_production: 58 max_consumption: 59 } network { resource { name: \"name\" } link { kind: urban target_id: 50 capacity: 51 flow: 52 } link { kind: urban target_id: 50 capacity: 51 flow: 52 } enabled: true max_stock: 53 stock: 54 production: 55 consumption: 56 critical: true initial_stock: 57 max_production: 58 max_consumption: 59 } } } perceived: true perceiving_automats { elem { id: 11 } elem { id: 12 } } knowledge_group { id: 13 } } }" ) );
    converter.ReceiveSimToClient( "unused endpoint", msg );
}
