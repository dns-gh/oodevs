...
BOOST_FIXTURE_TEST_CASE( min_range_to_fire_on_with_posture_is_zero_when_weapon_does_not_exist, EnemyFixture )
{
    ...
    MOCK_EXPECT( Log ).once().with( SWORD_LOG_LEVEL_ERROR,
        "Exception in GetMinRangeToFireOnWithPosture hook: Unknown weapon type : " + boost::lexical_cast< std::string >( invalid ) );
...
BOOST_FIXTURE_TEST_CASE( min_range_to_fire_on_with_posture_is_zero_when_volume_is_invalid, EnemyFixture )
{
    ...
    MOCK_EXPECT( Log ).once().with( SWORD_LOG_LEVEL_ERROR,
        "Exception in GetMinRangeToFireOnWithPosture hook: Invalid target volume identifier in GetMinDistanceForPH : " + boost::lexical_cast< std::string >( invalid ) );
    BOOST_CHECK_EQUAL( 0, GetMinRangeToFireOnWithPosture( firer, enemy, &filter, 0.5 ) );
}
