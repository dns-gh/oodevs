...
BOOST_AUTO_TEST_CASE( netn_ready_to_receive_service_registration_publish_only )
{
    ...
    MOCK_EXPECT( federate, RegisterInteraction ).once().with( "NETN_Service.NETN_ReadyToReceiveService", mock::any, true, false ).calls( boost::bind( &hla::Interaction_ABC::SetHandler, _2, boost::ref( *handler ) ) );
    NetnReadyToReceiveService interaction( federate );
}
