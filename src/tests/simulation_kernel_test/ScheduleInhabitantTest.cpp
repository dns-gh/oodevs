...
namespace
{
    ...
    unsigned int Convert( unsigned short year, unsigned short month, unsigned short day, unsigned short hour, unsigned short minute )
    {
        boost::gregorian::date date( year, month, day );
        boost::posix_time::time_duration time( hour, minute, 0u );
...
{
    schedule.Update( Convert( 2011, 1, 3, 11, 15 ) );
}

BOOST_FIXTURE_TEST_CASE( wrong_day_does_nothing, Fixture )
{
    schedule.Update( Convert( 2011, 1, 4, 22, 0 ) );
}
...
{
    {
        ...
        schedule.Update( Convert( 2011, 1, 17, 22, 0 ) );
    }
    MOCK_EXPECT( livingArea, StartMotivation ).once().with( "office" );
    schedule.Update( Convert( 2011, 1, 21, 9, 0 ) );
}
...
BOOST_FIXTURE_TEST_CASE( every_week_schedule_notifies_living_area, Fixture )
{
    {
        MOCK_EXPECT( livingArea, StartMotivation ).once().with( "leisure" );
        schedule.Update( Convert( 2011, 1, 3, 22, 0 ) );
    }
    {
        MOCK_EXPECT( livingArea, StartMotivation ).once().with( "leisure" );
        schedule.Update( Convert( 2011, 1, 10, 22, 0 ) );
    }
    MOCK_EXPECT( livingArea, StartMotivation ).once().with( "leisure" );
    schedule.Update( Convert( 2011, 1, 17, 22, 0 ) );
}
