...
namespace
{
    struct Fixture
    {
        ...
        {
            MOCK_FUNCTOR( bool( const MT_Vector2D& ) ) collector;
            MOCK_EXPECT( collector, _ ).once().with( to ).returns( false );
        ...
        void TestConsecutive( const MT_Vector2D& from, const MT_Vector2D& middle, const MT_Vector2D& to )
        {
            mock::sequence s;
            MOCK_FUNCTOR( bool( const MT_Vector2D& ) ) collector;
            MOCK_EXPECT( collector, _ ).once().with( middle ).in( s ).returns( false );
            MOCK_EXPECT( collector, _ ).once().with( to ).in( s ).returns( false );
            elevation.FindPath( from, to, collector );
        }
        int size;
        Elevation elevation;
