...
(function() {
  ...
  validate_number = function(data, ui, id, min, max, tab, msg) {
    var val, widget;
    widget = ui.find("#" + id);
  ...
  validate_double = function(data, ui, id, min, max, tab, msg) {
    var val, widget;
    widget = ui.find("#" + id);
  ...
  validate_rng = function(data, ui, type) {
    ...
    if (!validate_double(data, ui, "rng_" + type + "_deviation", 1e-32, Number.MAX_VALUE, "tab_rng", "Invalid")) {
      tab.tab('show');
      return false;
    }
    if (!validate_double(data, ui, "rng_" + type + "_mean", 1e-32, Number.MAX_VALUE, "tab_rng", "Invalid")) {
      tab.tab('show');
      return false;
    }
    return true;
  ...
  validate_settings = function(ui) {
    ...
    data.name = name.val();
    if (!validate_number(data, ui, "checkpoints_frequency", 1, 100, "tab_checkpoints", "[1, 100] Only")) {
      return;
    }
    if (!validate_number(data, ui, "checkpoints_keep", 1, 100, "tab_checkpoints", "[1, 100] Only")) {
      return;
    }
    data.checkpoints_enabled = ui.find("#checkpoints_enabled").is(":checked");
    if (!validate_number(data, ui, "time_step", 1, Number.MAX_VALUE, "tab_time", "Invalid")) {
      return;
    }
    if (!validate_number(data, ui, "time_factor", 1, Number.MAX_VALUE, "tab_time", "Invalid")) {
      return;
    }
    if (!validate_number(data, ui, "time_end_tick", 0, Number.MAX_VALUE, "tab_time", "Invalid")) {
      return;
    }
    data.time_paused = ui.find("#time_paused").is(":checked");
    if (!validate_number(data, ui, "rng_seed", 0, Number.MAX_VALUE, "tab_rng", "Invalid")) {
      return;
    }
    _ref = ["fire", "wound", "perception", "breakdown"];
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      it = _ref[_i];
      if (!validate_rng(data, ui, it)) {
        return;
      }
    }
    if (!validate_number(data, ui, "pathfind_threads", 0, 8, "tab_advanced", "[0, 8] Only")) {
      return;
    }
    if (!validate_number(data, ui, "recorder_frequency", 1, Number.MAX_VALUE, "Invalid")) {
      return;
    }
    return data;
  };
  ...
  SessionItemView = (function(_super) {
    ...
    SessionItemView.prototype.edit = function(evt) {
      ...
      return mod.find(".apply").click(function() {
        ...
        mod.modal("hide");
        return ajax("/api/update_session", data, function(attr) {
          return _this.model.set(attr);
        }, function() {
